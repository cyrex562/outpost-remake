...
static CRITICAL_SECTION_DEBUG critsect_debug =
    {
        0, 0, &display_dc_section, {&critsect_debug.ProcessLocksList, &critsect_debug.ProcessLocksList}, 0, 0, {(DWORD_PTR)(__FILE__ ": display_dc_section")}};
...
{
    BOOL (*get)
    (union sysparam_all_entry *entry, UINT int_param, void *ptr_param, UINT dpi);
    BOOL (*set)
    (union sysparam_all_entry *entry, UINT int_param, void *ptr_param, UINT flags);
    BOOL (*init)
    (union sysparam_all_entry *entry);
    const WCHAR *regval;
    const WCHAR *mirror;
    BOOL loaded;
};
...
static BOOL get_base_keys(enum parameter_key index, HKEY *base_key, HKEY *volatile_key)
{
    ...
    {
        if (RegCreateKeyW(HKEY_CURRENT_USER, parameter_key_names[index], &key))
            return FALSE;
    ...
    if (!volatile_keys[index] && volatile_key)
    {
        if (RegCreateKeyExW(volatile_base_key, parameter_key_names[index],
                            0, 0, REG_OPTION_VOLATILE, KEY_ALL_ACCESS, NULL, &key, 0))
            return FALSE;
    ...
    if (volatile_key)
        *volatile_key = volatile_keys[index];
    return TRUE;
}
...
static DWORD load_entry(struct sysparam_entry *entry, void *data, DWORD size)
{
    ...
    if (!get_base_keys(entry->regval[0], &base_key, &volatile_key))
        return FALSE;
    ...
    {
        count = size;
        if (RegQueryValueExW(base_key, entry->regval + 1, NULL, &type, data, &count))
            count = 0;
    }
    /* make sure strings are null-terminated */
    if (size && count == size && type == REG_SZ)
        ((WCHAR *)data)[count / sizeof(WCHAR) - 1] = 0;
    entry->loaded = TRUE;
    return count;
}
                       ...
                       DWORD type, UINT flags)
{
    ...
    if (flags & SPIF_UPDATEINIFILE)
    {
        if (!get_base_keys(entry->regval[0], &base_key, &volatile_key))
            return FALSE;
        if (RegSetValueExW(base_key, entry->regval + 1, 0, type, data, size))
            return FALSE;
    ...
    {
        if (!get_base_keys(entry->regval[0], NULL, &volatile_key))
            return FALSE;
        if (RegSetValueExW(volatile_key, entry->regval + 1, 0, type, data, size))
            return FALSE;
    }
    return TRUE;
}
...
static BOOL init_entry(struct sysparam_entry *entry, const void *data, DWORD size, DWORD type)
{
    ...
    if (!get_base_keys(entry->regval[0], &base_key, NULL))
        return FALSE;
    if (!RegQueryValueExW(base_key, entry->regval + 1, NULL, NULL, NULL, NULL))
        return TRUE;
    if (RegSetValueExW(base_key, entry->regval + 1, 0, type, data, size))
        return FALSE;
    if (entry->mirror && get_base_keys(entry->mirror[0], &base_key, NULL))
...
HDC get_display_dc(void)
{
    ...
    if (!display_dc)
        display_dc = CreateDCW(DISPLAY, NULL, NULL, NULL);
    return display_dc;
}
...
static void normalize_nonclientmetrics(NONCLIENTMETRICSW *pncm)
{
    ...
    if (pncm->iBorderWidth < 1)
        pncm->iBorderWidth = 1;
    if (pncm->iCaptionWidth < 8)
        pncm->iCaptionWidth = 8;
    if (pncm->iScrollWidth < 8)
        pncm->iScrollWidth = 8;
    if (pncm->iScrollHeight < 8)
        pncm->iScrollHeight = 8;
...
static BOOL set_uint_entry(union sysparam_all_entry *entry, UINT int_param, void *ptr_param, UINT flags)
{
    ...
    if (!save_entry_string(&entry->hdr, buf, flags))
        return FALSE;
...
static BOOL set_yesno_entry(union sysparam_all_entry *entry, UINT int_param, void *ptr_param, UINT flags)
{
    ...
    if (!save_entry_string(&entry->hdr, str, flags))
        return FALSE;
    entry->bool.val = int_param != 0;
    entry->hdr.loaded = TRUE;
    return TRUE;
...
static BOOL get_dword_entry(union sysparam_all_entry *entry, UINT int_param, void *ptr_param, UINT dpi)
{
    ...
    {
        DWORD val;
        if (load_entry(&entry->hdr, &val, sizeof(val)) == sizeof(DWORD))
            entry->dword.val = val;
    }
...
static BOOL set_dword_entry(union sysparam_all_entry *entry, UINT int_param, void *ptr_param, UINT flags)
{
    ...
    if (!save_entry(&entry->hdr, &val, sizeof(val), REG_DWORD, flags))
        return FALSE;
    entry->dword.val = val;
    entry->hdr.loaded = TRUE;
    return TRUE;
...
static BOOL set_font_entry(union sysparam_all_entry *entry, UINT int_param, void *ptr_param, UINT flags)
{
    ...
    if (!save_entry(&entry->hdr, &font, sizeof(font), REG_BINARY, flags))
        return FALSE;
    entry->font.val = font;
    entry->hdr.loaded = TRUE;
    return TRUE;
...
static BOOL get_userpref_entry(union sysparam_all_entry *entry, UINT int_param, void *ptr_param, UINT dpi)
{
    ...
    if (!parent_entry->hdr.get(parent_entry, sizeof(prefs), prefs, dpi))
        return FALSE;
...
static BOOL set_userpref_entry(union sysparam_all_entry *entry, UINT int_param, void *ptr_param, UINT flags)
{
    ...
    if (!parent_entry->hdr.get(parent_entry, sizeof(prefs), prefs, GetDpiForSystem()))
        return FALSE;

    if (PtrToUlong(ptr_param))
        prefs[entry->pref.offset] |= entry->pref.mask;
    else
        prefs[entry->pref.offset] &= ~entry->pref.mask;
...
static struct sysparam_rgb_entry system_colors[] =
    {
#define RGB_ENTRY(name, val)                                                  \
    {                                                                         \
        {get_rgb_entry, set_rgb_entry, init_rgb_entry, name##_VALNAME}, (val) \
    }
...
void SYSPARAMS_Init(void)
{
    ...
    if (!system_dpi)
        system_dpi = USER_DEFAULT_SCREEN_DPI;
...
static BOOL update_desktop_wallpaper(void)
{
    ...
    if (GetWindowThreadProcessId(GetDesktopWindow(), &pid) && pid == GetCurrentProcessId())
    {
        ...
        SendMessageW(GetDesktopWindow(), WM_SETTINGCHANGE, SPI_SETDESKWALLPAPER, 0);
    return TRUE;
}
...
BOOL WINAPI SetSysColors(INT count, const INT *colors, const COLORREF *values)
{
    ...
    if (IS_INTRESOURCE(colors))
        return FALSE; /* stupid app passes a color instead of an array */
                                     ...
                                     DWORD flags, LPVOID lparam)
{
    ...
    if (devname)
        RtlCreateUnicodeStringFromAsciiz(&nameW, devname);
    else
        nameW.Buffer = NULL;
                                   ...
                                   LPDEVMODEA lpDevMode, DWORD dwFlags)
{
    ...
    if (lpszDeviceName)
        RtlCreateUnicodeStringFromAsciiz(&nameW, lpszDeviceName);
    else
        nameW.Buffer = NULL;
...
DPI_AWARENESS_CONTEXT WINAPI GetThreadDpiAwarenessContext(void)
{
    ...
    if (info->dpi_awareness)
        return ULongToHandle(info->dpi_awareness);
    if (dpi_awareness)
...
DPI_AWARENESS_CONTEXT WINAPI SetThreadDpiAwarenessContext(DPI_AWARENESS_CONTEXT context)
{
    ...
    if (!(prev = info->dpi_awareness))
    {
        prev = dpi_awareness;
        if (!prev)
            prev = 0x10 | DPI_AWARENESS_UNAWARE;
        prev |= 0x80000000; /* restore to process default */
    }
    if (((ULONG_PTR)context & ~(ULONG_PTR)0x13) == 0x80000000)
        info->dpi_awareness = 0;
    else
        info->dpi_awareness = val | 0x10;
