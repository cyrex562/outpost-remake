...
{
    if (message >= 8 * sizeof(message_pointer_flags))
        return FALSE;
    if (message == WM_DEVICECHANGE && !(wparam & 0x8000))
        return FALSE;
...
{
    if (message >= 8 * sizeof(message_unicode_flags))
        return FALSE;
...
{
    if (!hwnd_filter || hwnd_filter == GetDesktopWindow())
        return TRUE;
...
static inline BOOL get_pending_wmchar(MSG *msg, UINT first, UINT last, BOOL remove)
{
    ...
    if (!data || !data->get_msg.message)
        return FALSE;
    if ((first || last) && (first > WM_CHAR || last < WM_CHAR))
        return FALSE;
    if (!msg)
        return FALSE;
    *msg = data->get_msg;
    if (remove)
        data->get_msg.message = 0;
    return TRUE;
}
...
static BOOL CALLBACK broadcast_message_callback(HWND hwnd, LPARAM lparam)
{
    ...
    if (!(GetWindowLongW(hwnd, GWL_STYLE) & (WS_POPUP | WS_CAPTION)))
        return TRUE;
    switch (info->type)
    {
    case MSG_UNICODE:
...
BOOL map_wparam_AtoW(UINT message, WPARAM *wparam, enum wm_char_mapping mapping)
{
    ...
    case WM_IME_CHAR:
        ...
        if (ch[0])
            ...
            MultiByteToWideChar(cp, 0, ch + 1, 1, wch, 1);
                           ...
                           struct packed_message *data)
{
    ...
    case WM_CREATE:
    {
        ...
        if (!IS_INTRESOURCE(cs->lpszName))
            push_string(data, cs->lpszName);
        if (!IS_INTRESOURCE(cs->lpszClass))
            push_string(data, cs->lpszClass);
    ...
    case WM_COPYDATA:
    {
        ...
        if (cds->lpData)
            push_data(data, cds->lpData, cds->cbData);
        return 0;
    }
    case WM_NOTIFY:
    ...
    case LB_SETTABSTOPS:
        if (wparam)
            push_data(data, lparam, sizeof(UINT) * wparam);
        return 0;
    case CB_ADDSTRING:
    ...
    case WM_MDICREATE:
    {
        ...
        if (!IS_INTRESOURCE(mcs->szClass))
            push_string(data, mcs->szClass);
        if (!IS_INTRESOURCE(mcs->szTitle))
            push_string(data, mcs->szTitle);
                           ...
                           void **buffer, size_t size)
{
    ...
    case WM_GETTEXT:
    case WM_ASKCBFORMATNAME:
        if (!get_buffer_space(buffer, (*wparam * sizeof(WCHAR))))
            return FALSE;
        break;
    case WM_WININICHANGE:
        if (!*lparam)
    ...
    case SBM_SETSCROLLINFO:
        ...
        break;
    case SBM_GETSCROLLINFO:
        if (!get_buffer_space(buffer, sizeof(SCROLLINFO)))
            return FALSE;
        break;
    case SBM_GETSCROLLBARINFO:
        if (!get_buffer_space(buffer, sizeof(SCROLLBARINFO)))
            return FALSE;
        break;
    case EM_GETSEL:
    case SBM_GETRANGE:
    case CB_GETEDITSEL:
        if (*wparam || *lparam)
        {
            if (!get_buffer_space(buffer, 2 * sizeof(DWORD)))
                return FALSE;
            if (*wparam)
                *wparam = (WPARAM)*buffer;
            if (*lparam)
                *lparam = (LPARAM)((DWORD *)*buffer + 1);
        }
        return TRUE;
    case EM_GETRECT:
    case LB_GETITEMRECT:
    case CB_GETDROPPEDCONTROLRECT:
        if (!get_buffer_space(buffer, sizeof(RECT)))
            return FALSE;
        break;
    case EM_SETRECT:
    case EM_SETRECTNP:
    ...
    case EM_GETLINE:
    {
        ...
        if (size < sizeof(WORD))
            return FALSE;
        len = *(WORD *)*buffer;
        if (!get_buffer_space(buffer, (len + 1) * sizeof(WCHAR)))
            return FALSE;
    ...
    case CB_GETLBTEXT:
    {
        ...
        if (!get_buffer_space(buffer, size))
            return FALSE;
        break;
    }
    case LB_GETTEXT:
    ...
    case LB_GETSELITEMS:
        if (!get_buffer_space(buffer, *wparam * sizeof(UINT)))
            return FALSE;
        break;
    case WM_NEXTMENU:
    {
    ...
    case WM_MDIGETACTIVE:
        if (!*lparam)
            return TRUE;
        if (!get_buffer_space(buffer, sizeof(BOOL)))
            return FALSE;
        break;
    case WM_DEVICECHANGE:
                       ...
                       LRESULT res, struct packed_message *data)
{
    ...
    case LB_GETSELITEMS:
        push_data(data, lparam, wparam * sizeof(UINT));
        break;
    case WM_MDIGETACTIVE:
        if (lparam)
            push_data(data, (BOOL *)lparam, sizeof(BOOL));
        break;
    case WM_NCCALCSIZE:
        if (!wparam)
            push_data(data, (RECT *)lparam, sizeof(RECT));
    ...
    case CB_GETEDITSEL:
        if (wparam)
            push_data(data, (DWORD *)wparam, sizeof(DWORD));
        if (lparam)
            push_data(data, (DWORD *)lparam, sizeof(DWORD));
        break;
    case WM_NEXTMENU:
    {
        MDINEXTMENU *mnm = (MDINEXTMENU *)lparam;
                         ...
                         void *buffer, size_t size)
{
    ...
    case EM_GETLINE:
        ...
        break;
    case LB_GETSELITEMS:
        memcpy(lparam, buffer, min(wparam * sizeof(UINT), size));
        break;
    case LB_GETTEXT:
    case CB_GETLBTEXT:
        memcpy((WCHAR *)lparam, buffer, size);
    ...
    case WM_MDIGETACTIVE:
        if (lparam)
            memcpy((BOOL *)lparam, buffer, min(sizeof(BOOL), size));
        break;
    case WM_NCCALCSIZE:
        if (!wparam)
            memcpy((RECT *)lparam, buffer, min(sizeof(RECT), size));
    ...
    case CB_GETEDITSEL:
        ...
        if (lparam)
            memcpy((DWORD *)lparam, buffer, min(sizeof(DWORD), size));
        break;
    case WM_MDICREATE:
        if (size >= sizeof(ps->mcs))
        {
...
static CRITICAL_SECTION_DEBUG critsect_debug =
    {
        0, 0, &dde_crst, {&critsect_debug.ProcessLocksList, &critsect_debug.ProcessLocksList}, 0, 0, {(DWORD_PTR)(__FILE__ ": dde_crst")}};
                               ...
                               void **buffer, size_t size)
{
    ...
    case WM_DDE_DATA:
    case WM_DDE_POKE:
        if ((!buffer || !*buffer) && message != WM_DDE_DATA)
            return FALSE;
        uiHi = *lparam;
        if (size)
        {
                                ...
                                BOOL unicode, BOOL same_thread, enum wm_char_mapping mapping)
{
    ...
    if (!WINPROC_call_window(hwnd, msg, wparam, lparam, &result, unicode, mapping))
        goto done;
...
static BOOL process_rawinput_message(MSG *msg, const struct hardware_msg_data *msg_data)
{
    ...
    if (!rawinput)
    {
        ...
        if (!(rawinput = thread_info->rawinput))
            return FALSE;
    }
        ...
        (msg->message == WM_XBUTTONDOWN))
    {
        ...
        if (msg->hwnd != info.hwndActive)
        {
            ...
            if ((GetWindowLongW(hwndTop, GWL_STYLE) & (WS_POPUP | WS_CHILD)) != WS_CHILD)
            {
                ...
                case MA_ACTIVATE:
                case 0:
                    if (!FOCUS_MouseActivate(hwndTop))
                        eatMsg = TRUE;
                    break;
                default:
...
static BOOL peek_message(MSG *msg, HWND hwnd, UINT first, UINT last, UINT flags, UINT changed_mask)
{
    ...
    for (;;)
    {
        ...
        if (HIWORD(flags) && !changed_mask)
            flags = PM_QS_SENDMESSAGE | LOWORD(flags);
    }
}
...
static BOOL put_message_in_queue(const struct send_message_info *info, size_t *reply_size)
{
    ...
    SERVER_START_REQ(send_message)
    {
        ...
        if (info->flags & SMTO_ABORTIFHUNG)
            req->flags |= SEND_MSG_ABORT_IF_HUNG;
        for (i = 0; i < data.count; i++)
            wine_server_add_data(req, data.data[i], data.size[i]);
        if ((res = wine_server_call(req)))
        {
                              ...
                              size_t reply_size, LRESULT *result)
{
    ...
    SERVER_START_REQ(get_message_reply)
    {
        req->cancel = 1;
        if (reply_size)
            wine_server_set_reply(req, reply_data, reply_size);
...
static LRESULT send_inter_thread_message(const struct send_message_info *info, LRESULT *res_ptr)
{
    ...
    if (!put_message_in_queue(info, &reply_size))
        return 0;

    /* there's no reply to wait for on notify/callback messages */
    if (info->type == MSG_NOTIFY || info->type == MSG_CALLBACK)
        return 1;
...
static BOOL send_message(struct send_message_info *info, DWORD_PTR *res_ptr, BOOL unicode)
{
    ...
    if (!(info->dest_tid = GetWindowThreadProcessId(info->hwnd, &dest_pid)))
        return FALSE;

    if (USER_IsExitingThread(info->dest_tid))
        return FALSE;
...
NTSTATUS send_hardware_message(HWND hwnd, const INPUT *input, UINT flags)
{
    ...
    SERVER_START_REQ(send_hardware_message)
    {
        ...
        if (key_state_info)
            wine_server_set_reply(req, key_state_info->state,
                                       ...
                                       UINT flags, UINT timeout, PDWORD_PTR res_ptr)
{
    ...
    {
        if (dest_pid != GetCurrentProcessId())
            info.type = MSG_OTHER_PROCESS;
        ret = send_inter_thread_message(&info, &result);
    }
    if (ret && res_ptr)
...
{
    if (!map_wparam_AtoW(msg, &wparam, WMCHAR_MAP_POSTMESSAGE))
        return TRUE;
...
BOOL WINAPI PostMessageW(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
    ...
    if (!(info.dest_tid = GetWindowThreadProcessId(hwnd, NULL)))
        return FALSE;
...
BOOL WINAPI PostThreadMessageW(DWORD thread, UINT msg, WPARAM wparam, LPARAM lparam)
{
    ...
    if (USER_IsExitingThread(thread))
        return TRUE;
...
BOOL WINAPI DECLSPEC_HOTPATCH PeekMessageW(MSG *msg_out, HWND hwnd, UINT first, UINT last, UINT flags)
{
    ...
    if (!peek_message(&msg, hwnd, first, last, flags, 0))
    {
        ...
        if (ret == WAIT_TIMEOUT || !peek_message(&msg, hwnd, first, last, flags, 0))
            return FALSE;
    }
...
{
    if (get_pending_wmchar(msg, first, last, (flags & PM_REMOVE)))
        return TRUE;
    if (!PeekMessageW(msg, hwnd, first, last, flags))
        return FALSE;
...
{
    if (get_pending_wmchar(msg, first, last, TRUE))
        return TRUE;
...
DWORD WINAPI WaitForInputIdle(HANDLE hProcess, DWORD dwTimeOut)
{
    ...
    SERVER_END_REQ;
    if (!handles[1])
        return WAIT_FAILED; /* no event to wait on */
...
LONG WINAPI BroadcastSystemMessageExW(DWORD flags, LPDWORD recipients, UINT msg, WPARAM wp, LPARAM lp, PBSMINFO pinfo)
{
    ...
    BOOL ret = TRUE;
    static const DWORD all_flags = (BSF_QUERY | BSF_IGNORECURRENTTASK | BSF_FLUSHDISK | BSF_NOHANG | BSF_POSTMESSAGE | BSF_FORCEIFHUNG | BSF_NOTIMEOUTIFNOTHUNG | BSF_ALLOWSFW | BSF_SENDNOTIFYMESSAGE | BSF_RETURNHDESK | BSF_LUID);
...
UINT_PTR WINAPI SetCoalescableTimer(HWND hwnd, UINT_PTR id, UINT timeout, TIMERPROC proc, ULONG tolerance)
{
    ...
    if (proc)
        winproc = WINPROC_AllocProc((WNDPROC)proc, FALSE);
...
BOOL WINAPI GetGUIThreadInfo(DWORD id, GUITHREADINFO *info)
{
    ...
    SERVER_START_REQ(get_thread_input)
    {
        ...
        if ((ret = !wine_server_call_err(req)))
        {
            ...
            if (reply->menu_owner)
                info->flags |= GUI_INMENUMODE;
            if (reply->move_size)
                info->flags |= GUI_INMOVESIZE;
            if (reply->caret)
                info->flags |= GUI_CARETBLINKING;
        }
    }
    SERVER_END_REQ;
    return ret;
