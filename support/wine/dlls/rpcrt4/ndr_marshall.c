                            ...
                            PFORMAT_STRING pFormat)
{
  ...
  if (pointer_needs_marshaling)
  {
    ...
    if (m)
      m(pStubMsg, Pointer, desc);
    else
      FIXME("no marshaller for data type=%02x\n", *desc);
  }
                              ...
                              unsigned char fMustAlloc)
{
  ...
  if (pointer_needs_unmarshaling)
  {
    ...
    if (attr & FC_POINTER_DEREF)
    {
      ...
      TRACE("deref => %p\n", current_ptr);
      if (!fMustAlloc && !*current_ptr)
        fMustAlloc = TRUE;
    }
    m = NdrUnmarshaller[*desc & NDR_TABLE_MASK];
    if (m)
      m(pStubMsg, current_ptr, desc, fMustAlloc);
    else
      FIXME("no unmarshaller for data type=%02x\n", *desc);
                              ...
                              PFORMAT_STRING pFormat)
{
  ...
  if (m)
    ...
    FIXME("no buffersizer for data type=%02x\n", *desc);
}
                        ...
                        PFORMAT_STRING pFormat)
{
  ...
  if (m)
    m(pStubMsg, current_pointer, desc);
...
#define BASE_TYPE_UNMARSHALL(type)                \
  ...
  *(type *)pMemory = *(type *)pStubMsg->Buffer;   \
  ...
  case FC_ENUM16:
    ...
    *pMemory = *(USHORT *)pStubMsg->Buffer;
  ...
  case FC_UINT3264:
    ...
    /* 32-bits on the wire, but int_ptr in memory */
    *(UINT_PTR *)pMemory = *pStubMsg->Buffer;
  ...
  while (*pFormat != FC_END)
  {
    ...
    case FC_EMBEDDED_COMPLEX:
      ...
      if (m)
      {
        ...
        FIXME("no unmarshaller for embedded type %02x\n", *desc);
      pMemory += size;
      pFormat += 2;
      continue;
    case FC_PAD:
                                               ...
                                               PFORMAT_STRING pFormat)
{
  ...
  pFormat += 4;
  if (*(const SHORT *)pFormat)
    conf_array = pFormat + *(const SHORT *)pFormat;
  pFormat += 2;
  if (*(const WORD *)pFormat)
    pointer_desc = pFormat + *(const WORD *)pFormat;
  pFormat += 2;
...
#define BASE_TYPE_UNMARSHALL(type)                              \
  ...
  case FC_INT3264:
    ...
    break;
  case FC_UINT3264:
    if (sizeof(UINT_PTR) == sizeof(UINT))
