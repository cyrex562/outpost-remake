...
static void test_basic_type(void)
{
    ...
    static const struct
    {
    ...
    } tests[] =
        {
            ...
            {WS_u16YPE, 65535, sizeof(UINT16), "<t>65535</t>", "<t t=\"65535\"/>"},
...
static void test_datetime(void)
{
    ...
    for (i = 0; i < ARRAY_SIZE(tests); i++)
    {
        ...
        if (hr != tests[i].hr && hr == tests[i].hr_broken)
            break;
        if (hr == S_OK)
        {
...
static void test_binary_encoding(void)
{
    ...
    for (i = 0; i < ARRAY_SIZE(elem_tests); i++)
    {
        ...
        ok(hr == S_OK, "%u: got %08x\n", i, hr);
        if (hr == S_OK)
            check_output_bin(writer, elem_tests[i].result, elem_tests[i].len_result, __LINE__);
    }
    ...
    for (i = 0; i < ARRAY_SIZE(attr_tests); i++)
    {
        ...
        ok(hr == S_OK, "%u: got %08x\n", i, hr);
        if (hr == S_OK)
            check_output_bin(writer, attr_tests[i].result, attr_tests[i].len_result, __LINE__);
    }
    ...
    ok(hr == S_OK, "got %08x\n", hr);
    if (hr == S_OK)
        check_output_bin(writer, res200, sizeof(res200), __LINE__);
