...
{
    if (is_valid_parent(writer->current))
        return writer->current;
    if (is_valid_parent(writer->current->parent))
        return writer->current->parent;
    return NULL;
}
                           ...
                           ULONG count, WS_ERROR *error)
{
    ...
    if (!(node = alloc_node(WS_XML_NODE_TYPE_BOF)))
        hr = E_OUTOFMEMORY;
    else
...
static HRESULT write_grow_buffer(struct writer *writer, ULONG size)
{
    ...
    if (!(tmp = ws_realloc(buf->heap, buf->bytes.bytes, buf->size, new_size)))
                                  ...
                                  const struct escape **escapes, ULONG nb_escapes)
{
    ...
    for (i = 0; i < len; i++)
    {
        ...
        if ((hr = write_grow_buffer(writer, size)) != S_OK)
            return hr;
...
static HRESULT write_attribute_text(struct writer *writer, const WS_XML_ATTRIBUTE *attr)
{
    ...
    if (attr->value)
        hr = write_attribute_value_text(writer, attr->value, attr->singleQuote);
    write_char(writer, quote);
...
static HRESULT write_int31(struct writer *writer, ULONG len)
{
    ...
    if ((hr = write_grow_buffer(writer, 1)) != S_OK)
        return hr;
    if (len < 0x80)
    {
...
{
    HRESULT hr;
    if ((hr = write_int31(writer, len)) != S_OK)
        return hr;
    if ((hr = write_grow_buffer(writer, len)) != S_OK)
        return hr;
...
static enum record_type get_attr_text_record_type(const WS_XML_TEXT *text, BOOL use_dict)
{
    ...
    case WS_XML_TEXT_TYPE_UTF8:
    {
        ...
        if (text_utf8->value.length <= MAX_UINT8)
            return RECORD_CHARS8_TEXT;
        if (text_utf8->value.length <= MAX_UINT16)
    ...
    case WS_XML_TEXT_TYPE_UTF16:
    {
        ...
        if (len_utf8 <= MAX_UINT8)
            return RECORD_CHARS8_TEXT;
        if (len_utf8 <= MAX_UINT16)
            return RECORD_CHARS16_TEXT;
        return RECORD_CHARS32_TEXT;
    }
    case WS_XML_TEXT_TYPE_BASE64:
    {
        ...
        if (text_base64->length <= MAX_UINT8)
            return RECORD_BYTES8_TEXT;
        if (text_base64->length <= MAX_UINT16)
    ...
    case WS_XML_TEXT_TYPE_INT32:
    {
        ...
        if (text_int32->value == 1)
            return RECORD_ONE_TEXT;
        if (text_int32->value >= MIN_INT8 && text_int32->value <= MAX_INT8)
            return RECORD_INT8_TEXT;
        if (text_int32->value >= MIN_INT16 && text_int32->value <= MAX_INT16)
    ...
    case WS_XML_TEXT_TYPE_INT64:
    {
        ...
        if (!text_int64->value)
            return RECORD_ZERO_TEXT;
        if (text_int64->value == 1)
            return RECORD_ONE_TEXT;
        if (text_int64->value >= MIN_INT8 && text_int64->value <= MAX_INT8)
            return RECORD_INT8_TEXT;
        if (text_int64->value >= MIN_INT16 && text_int64->value <= MAX_INT16)
            return RECORD_INT16_TEXT;
        if (text_int64->value >= MIN_INT32 && text_int64->value <= MAX_INT32)
            return RECORD_INT32_TEXT;
    ...
    case WS_XML_TEXT_TYPE_UINT64:
    {
        ...
        if (!text_uint64->value)
            return RECORD_ZERO_TEXT;
        if (text_uint64->value == 1)
            return RECORD_ONE_TEXT;
        if (text_uint64->value <= MAX_INT8)
            return RECORD_INT8_TEXT;
        if (text_uint64->value <= MAX_INT16)
            return RECORD_INT16_TEXT;
        if (text_uint64->value <= MAX_INT32)
            return RECORD_INT32_TEXT;
        if (text_uint64->value <= MAX_INT64)
            return RECORD_INT64_TEXT;
    ...
    case WS_XML_TEXT_TYPE_DOUBLE:
    {
        ...
        if (!text_double->value)
            return RECORD_ZERO_TEXT;
        if (text_double->value == 1)
            return RECORD_ONE_TEXT;
        if (isinf(text_double->value) || (INT64)text_double->value != text_double->value)
            return RECORD_DOUBLE_TEXT;
        if (text_double->value <= MAX_INT8)
            return RECORD_INT8_TEXT;
        if (text_double->value <= MAX_INT16)
            return RECORD_INT16_TEXT;
        if (text_double->value <= MAX_INT32)
            return RECORD_INT32_TEXT;
        return RECORD_INT64_TEXT;
    }
                         ...
                         WS_XML_UTF8_TEXT **ret)
{
    ...
    case WS_XML_TEXT_TYPE_UTF8:
    {
        ...
        if (!(*ret = alloc_utf8_text(NULL, len_old + src->value.length)))
            return E_OUTOFMEMORY;
        if (old)
            memcpy((*ret)->value.bytes, old->value.bytes, len_old);
    ...
    case WS_XML_TEXT_TYPE_UTF16:
    {
        ...
        if (src->byteCount % sizeof(WCHAR))
            return E_INVALIDARG;
        len_utf8 = WideCharToMultiByte(CP_UTF8, 0, str, len, NULL, 0, NULL, NULL);
        if (!(*ret = alloc_utf8_text(NULL, len_old + len_utf8)))
            return E_OUTOFMEMORY;
        if (old)
            memcpy((*ret)->value.bytes, old->value.bytes, len_old);
    ...
    case WS_XML_TEXT_TYPE_BASE64:
    {
        ...
        if (!(*ret = alloc_utf8_text(NULL, len_old + len)))
            return E_OUTOFMEMORY;
        if (old)
            memcpy((*ret)->value.bytes, old->value.bytes, len_old);
    ...
    case WS_XML_TEXT_TYPE_BOOL:
    {
        ...
        if (!(*ret = alloc_utf8_text(NULL, len_old + 5)))
            return E_OUTOFMEMORY;
        if (old)
            memcpy((*ret)->value.bytes, old->value.bytes, len_old);
    ...
    case WS_XML_TEXT_TYPE_GUID:
    {
        ...
        if (!(*ret = alloc_utf8_text(NULL, len_old + 37)))
            return E_OUTOFMEMORY;
        if (old)
            memcpy((*ret)->value.bytes, old->value.bytes, len_old);
    ...
    case WS_XML_TEXT_TYPE_UNIQUE_ID:
    {
        ...
        if (!(*ret = alloc_utf8_text(NULL, len_old + 46)))
            return E_OUTOFMEMORY;
        if (old)
            memcpy((*ret)->value.bytes, old->value.bytes, len_old);
    ...
    case WS_XML_TEXT_TYPE_DATETIME:
    {
        ...
        if (!(*ret = alloc_utf8_text(NULL, len_old + 34)))
            return E_OUTOFMEMORY;
        if (old)
            memcpy((*ret)->value.bytes, old->value.bytes, len_old);
    ...
    case WS_XML_TEXT_TYPE_QNAME:
    {
        ...
        if (qn->prefix && qn->prefix->length)
            len += qn->prefix->length + 1;
        if (!(*ret = alloc_utf8_text(NULL, len_old + len)))
            return E_OUTOFMEMORY;
        if (old)
            memcpy((*ret)->value.bytes, old->value.bytes, len_old);
...
static HRESULT write_attribute_value_bin(struct writer *writer, const WS_XML_TEXT *text)
{
    ...
    case RECORD_CHARS8_TEXT:
    {
        ...
        if (text->textType == WS_XML_TEXT_TYPE_UTF8)
            text_utf8 = (const WS_XML_UTF8_TEXT *)text;
        else
        {
            if ((hr = text_to_utf8text(text, NULL, NULL, &new)) != S_OK)
                return hr;
            text_utf8 = new;
        }
    ...
    case RECORD_BYTES8_TEXT:
    {
        ...
        if ((hr = write_grow_buffer(writer, 1 + text_base64->length)) != S_OK)
            return hr;
    ...
    case RECORD_INT8_TEXT:
    {
        ...
        if ((hr = write_grow_buffer(writer, sizeof(val))) != S_OK)
            return hr;
    ...
    case RECORD_UINT64_TEXT:
    {
        ...
        if ((hr = write_grow_buffer(writer, sizeof(text_uint64->value))) != S_OK)
            return hr;
    ...
    case RECORD_DOUBLE_TEXT:
    {
        ...
        if ((hr = write_grow_buffer(writer, sizeof(text_double->value))) != S_OK)
            return hr;
    ...
    case RECORD_GUID_TEXT:
    {
        ...
        if ((hr = write_grow_buffer(writer, sizeof(text_guid->value))) != S_OK)
            return hr;
    ...
    case RECORD_UNIQUE_ID_TEXT:
    {
        ...
        if ((hr = write_grow_buffer(writer, sizeof(text_unique_id->value))) != S_OK)
            return hr;
    ...
    case RECORD_DATETIME_TEXT:
    {
        ...
        if (text_datetime->value.format == WS_DATETIME_FORMAT_UTC)
            val |= (UINT64)1 << 62;
        else if (text_datetime->value.format == WS_DATETIME_FORMAT_LOCAL)
            val |= (UINT64)1 << 63;
...
static enum record_type get_attr_record_type(const WS_XML_ATTRIBUTE *attr, BOOL use_dict)
{
    ...
    {
        if (use_dict)
            return RECORD_PREFIX_DICTIONARY_ATTRIBUTE_A + attr->prefix->bytes[0] - 'a';
...
static HRESULT write_attribute_bin(struct writer *writer, const WS_XML_ATTRIBUTE *attr)
{
    ...
    {
        if ((hr = write_string(writer, attr->localName->bytes, attr->localName->length)) != S_OK)
            return hr;
    ...
    {
        if ((hr = write_dict_string(writer, id)) != S_OK)
            return hr;
        return write_attribute_value_bin(writer, attr->value);
    }
    ...
    case RECORD_ATTRIBUTE:
        if ((hr = write_string(writer, attr->prefix->bytes, attr->prefix->length)) != S_OK)
            return hr;
        if ((hr = write_string(writer, attr->localName->bytes, attr->localName->length)) != S_OK)
            return hr;
        break;
...
static HRESULT write_attribute(struct writer *writer, const WS_XML_ATTRIBUTE *attr)
{
    ...
    {
    case WS_XML_WRITER_ENCODING_TYPE_TEXT:
        return write_attribute_text(writer, attr);
    case WS_XML_WRITER_ENCODING_TYPE_BINARY:
        return write_attribute_bin(writer, attr);
    default:
...
static HRESULT write_namespace_attribute(struct writer *writer, const WS_XML_ATTRIBUTE *attr)
{
    ...
    {
    case WS_XML_WRITER_ENCODING_TYPE_TEXT:
        return write_namespace_attribute_text(writer, attr);
    case WS_XML_WRITER_ENCODING_TYPE_BINARY:
        return write_namespace_attribute_bin(writer, attr);
    default:
        ERR("unhandled encoding %u\n", writer->output_enc);
        return WS_E_NOT_SUPPORTED;
    }
                                       ...
                                       const WS_XML_STRING *ns, BOOL single)
{
    ...
    if (!(attr = heap_alloc_zero(sizeof(*attr))))
        return E_OUTOFMEMORY;
                               ...
                               const WS_XML_STRING *ns)
{
    ...
    for (node = (const struct node *)elem; node; node = node->parent)
    {
        ...
        {
            if (!elem->attributes[i]->isXmlNs)
                continue;
            if (str_equal(elem->attributes[i]->prefix, prefix) &&
                str_equal(elem->attributes[i]->ns, ns))
                return TRUE;
        }
    }
    return FALSE;
...
{
    WS_XML_STRING *str;
    if (!(str = dup_xml_string(ns, writer->dict_do_lookup)))
        return E_OUTOFMEMORY;
    free_xml_string(writer->current_ns);
...
static HRESULT set_namespaces(struct writer *writer)
{
    ...
    {
        if ((hr = add_namespace_attribute(writer, elem->prefix, elem->ns, FALSE)) != S_OK)
            return hr;
        if ((hr = set_current_namespace(writer, elem->ns)) != S_OK)
            return hr;
    }
    ...
    for (i = 0; i < elem->attributeCount; i++)
    {
        ...
        if (!attr->ns->length || namespace_in_scope(elem, attr->prefix, attr->ns))
            continue;
        if ((hr = add_namespace_attribute(writer, attr->prefix, attr->ns, FALSE)) != S_OK)
            return hr;
    }
...
static HRESULT write_attributes(struct writer *writer, const WS_XML_ELEMENT_NODE *elem)
{
    ...
    for (i = 0; i < elem->attributeCount; i++)
    {
        if (elem->attributes[i]->isXmlNs)
            continue;
        if ((hr = write_attribute(writer, elem->attributes[i])) != S_OK)
            return hr;
    }
    for (i = 0; i < elem->attributeCount; i++)
    {
        if (!elem->attributes[i]->isXmlNs || !elem->attributes[i]->prefix)
            continue;
        if ((hr = write_namespace_attribute(writer, elem->attributes[i])) != S_OK)
            return hr;
    }
    for (i = 0; i < elem->attributeCount; i++)
    {
...
static enum record_type get_elem_record_type(const WS_XML_ELEMENT_NODE *elem, BOOL use_dict)
{
    ...
    {
        if (use_dict)
            return RECORD_PREFIX_DICTIONARY_ELEMENT_A + elem->prefix->bytes[0] - 'a';
...
static HRESULT write_startelement_bin(struct writer *writer)
{
    ...
    {
        if ((hr = write_string(writer, elem->localName->bytes, elem->localName->length)) != S_OK)
            return hr;
        return write_attributes(writer, elem);
    }
    ...
    case RECORD_ELEMENT:
        if ((hr = write_string(writer, elem->prefix->bytes, elem->prefix->length)) != S_OK)
            return hr;
        if ((hr = write_string(writer, elem->localName->bytes, elem->localName->length)) != S_OK)
            return hr;
        break;
...
static HRESULT write_endelement_text(struct writer *writer, const WS_XML_ELEMENT_NODE *elem)
{
    ...
    {
        if ((hr = write_grow_buffer(writer, 2)) != S_OK)
            return hr;
        write_char(writer, '/');
        write_char(writer, '>');
        return S_OK;
...
static HRESULT write_endelement(struct writer *writer, const WS_XML_ELEMENT_NODE *elem)
{
    ...
    {
    case WS_XML_WRITER_ENCODING_TYPE_TEXT:
        return write_endelement_text(writer, elem);
    case WS_XML_WRITER_ENCODING_TYPE_BINARY:
...
static HRESULT write_endelement_node(struct writer *writer)
{
    ...
    if (!(node = write_find_startelement(writer)))
        return WS_E_INVALID_FORMAT;
    if (writer->state == WRITER_STATE_STARTELEMENT)
    {
        if ((hr = set_namespaces(writer)) != S_OK)
            return hr;
        if ((hr = write_startelement(writer)) != S_OK)
            return hr;
    }
    if ((hr = write_close_element(writer, node)) != S_OK)
        return hr;
...
HRESULT WINAPI WsWriteEndStartElement(WS_XML_WRITER *handle, WS_ERROR *error)
{
    ...
    if ((hr = write_startelement(writer)) != S_OK)
        goto done;
    if ((hr = write_endstartelement(writer)) != S_OK)
        goto done;
...
static HRESULT write_add_cdata_node(struct writer *writer)
{
    ...
    if (!(parent = find_parent(writer)))
        return WS_E_INVALID_FORMAT;
    if (!(node = alloc_node(WS_XML_NODE_TYPE_CDATA)))
        return E_OUTOFMEMORY;
...
{
    struct node *node;
    if (!(node = alloc_node(WS_XML_NODE_TYPE_END_CDATA)))
        return E_OUTOFMEMORY;
...
{
    HRESULT hr;
    if ((hr = write_grow_buffer(writer, 9)) != S_OK)
        return hr;
...
{
    HRESULT hr;
    if ((hr = write_flush(writer)) != S_OK)
        return hr;
    if ((hr = write_add_cdata_node(writer)) != S_OK)
        return hr;
    if ((hr = write_add_endcdata_node(writer)) != S_OK)
        return hr;
    if ((hr = write_cdata(writer)) != S_OK)
        return hr;
...
{
    HRESULT hr;
    if ((hr = write_grow_buffer(writer, 3)) != S_OK)
        return hr;
...
{
    HRESULT hr;
    if ((hr = write_endcdata(writer)) != S_OK)
        return hr;
                                      ...
                                      const WS_XML_STRING *localname, const WS_XML_STRING *ns)
{
    ...
    {
        elem = &parent->hdr;
        if (WsXmlStringEquals(ns, elem->ns, NULL) == S_OK)
            prefix = elem->prefix;
    }

    if (!(node = alloc_node(WS_XML_NODE_TYPE_ELEMENT)))
        return E_OUTOFMEMORY;
    elem = &node->hdr;
...
{
    struct node *node;
    if (!(node = alloc_node(WS_XML_NODE_TYPE_END_ELEMENT)))
        return E_OUTOFMEMORY;
    node->parent = parent;
                                  ...
                                  const WS_XML_STRING *localname, const WS_XML_STRING *ns)
{
    ...
    if ((hr = write_flush(writer)) != S_OK)
        return hr;
    if ((hr = write_add_element_node(writer, prefix, localname, ns)) != S_OK)
        return hr;
    if ((hr = write_add_endelement_node(writer, writer->current)) != S_OK)
        return hr;
    writer->state = WRITER_STATE_STARTELEMENT;
    return S_OK;
}
...
HRESULT text_to_text(const WS_XML_TEXT *text, const WS_XML_TEXT *old, ULONG *offset, WS_XML_TEXT **ret)
{
    ...
    case WS_XML_TEXT_TYPE_UTF8:
    {
        ...
        if (!(new = alloc_utf8_text(NULL, len_old + len)))
            return E_OUTOFMEMORY;
        if (utf8_old)
            memcpy(new->value.bytes, utf8_old->value.bytes, len_old);
    ...
    case WS_XML_TEXT_TYPE_UTF16:
    {
        ...
        if (utf16->byteCount % sizeof(WCHAR))
            return E_INVALIDARG;
        if (!(new = alloc_utf16_text(NULL, len_old + len)))
            return E_OUTOFMEMORY;
        if (utf16_old)
            memcpy(new->bytes, utf16_old->bytes, len_old);
    ...
    case WS_XML_TEXT_TYPE_BASE64:
    {
        ...
        if (!(new = alloc_base64_text(NULL, len_old + len)))
            return E_OUTOFMEMORY;
        if (base64_old)
            memcpy(new->bytes, base64_old->bytes, len_old);
    ...
    case WS_XML_TEXT_TYPE_BOOL:
    {
        ...
        if (!(new = alloc_bool_text(bool_text->value)))
            return E_OUTOFMEMORY;
        *ret = &new->text;
        return S_OK;
    }
    case WS_XML_TEXT_TYPE_INT32:
    {
        ...
        if (!(new = alloc_int32_text(int32_text->value)))
            return E_OUTOFMEMORY;
        *ret = &new->text;
        return S_OK;
    }
    case WS_XML_TEXT_TYPE_INT64:
    {
        ...
        if (!(new = alloc_int64_text(int64_text->value)))
            return E_OUTOFMEMORY;
        *ret = &new->text;
        return S_OK;
    }
    case WS_XML_TEXT_TYPE_UINT64:
    {
        ...
        if (!(new = alloc_uint64_text(uint64_text->value)))
            return E_OUTOFMEMORY;
        *ret = &new->text;
        return S_OK;
    }
    case WS_XML_TEXT_TYPE_DOUBLE:
    {
        ...
        if (!(new = alloc_double_text(double_text->value)))
            return E_OUTOFMEMORY;
        *ret = &new->text;
        return S_OK;
    }
    case WS_XML_TEXT_TYPE_GUID:
    {
        ...
        if (!(new = alloc_guid_text(&id->value)))
            return E_OUTOFMEMORY;
        *ret = &new->text;
        return S_OK;
    }
    case WS_XML_TEXT_TYPE_UNIQUE_ID:
    {
        ...
        if (!(new = alloc_unique_id_text(&id->value)))
            return E_OUTOFMEMORY;
        *ret = &new->text;
        return S_OK;
    }
    case WS_XML_TEXT_TYPE_DATETIME:
    {
        ...
        if (!(new = alloc_datetime_text(&dt->value)))
            return E_OUTOFMEMORY;
        *ret = &new->text;
        return S_OK;
    }
...
static HRESULT write_set_attribute_value(struct writer *writer, const WS_XML_TEXT *value)
{
    ...
    case WS_XML_TEXT_TYPE_UNIQUE_ID:
    case WS_XML_TEXT_TYPE_DATETIME:
        if (elem->attributes[elem->attributeCount - 1]->value)
            return WS_E_INVALID_OPERATION;
        break;
    ...
    case WS_XML_WRITER_ENCODING_TYPE_TEXT:
    {
        ...
        if ((hr = text_to_utf8text(value, old, NULL, &new)) != S_OK)
            return hr;
        heap_free(old);
    ...
    case WS_XML_WRITER_ENCODING_TYPE_BINARY:
    {
        ...
        if ((hr = text_to_text(value, old, NULL, &new)) != S_OK)
            return hr;
        heap_free(old);
...
static HRESULT write_add_text_node(struct writer *writer, const WS_XML_TEXT *value)
{
    ...
    if (node_type(writer->current) != WS_XML_NODE_TYPE_ELEMENT &&
        ...
        node_type(writer->current) != WS_XML_NODE_TYPE_CDATA)
        return WS_E_INVALID_FORMAT;
...
static HRESULT write_text_text(struct writer *writer, const WS_XML_TEXT *text, ULONG offset)
{
    ...
    {
        if ((hr = write_grow_buffer(writer, utf8->value.length - offset)) != S_OK)
            return hr;
...
static enum record_type get_text_record_type(const WS_XML_TEXT *text, BOOL use_dict)
{
    ...
    case WS_XML_TEXT_TYPE_INT32:
    {
        ...
        if (text_int32->value == 1)
            return RECORD_ONE_TEXT_WITH_ENDELEMENT;
        if (text_int32->value >= MIN_INT8 && text_int32->value <= MAX_INT8)
...
static HRESULT write_text_bin(struct writer *writer, const WS_XML_TEXT *text, ULONG offset)
{
    ...
    {
        INT8 val = get_text_value_int(text);
        if ((hr = write_grow_buffer(writer, 1 + sizeof(val))) != S_OK)
            return hr;
        write_char(writer, type);
        write_char(writer, val);
        return S_OK;
    ...
    {
        WS_XML_UINT64_TEXT *text_uint64 = (WS_XML_UINT64_TEXT *)text;
        if ((hr = write_grow_buffer(writer, 1 + sizeof(text_uint64->value))) != S_OK)
            return hr;
        write_char(writer, type);
        write_bytes(writer, (const BYTE *)&text_uint64->value, sizeof(text_uint64->value));
        return S_OK;
    ...
    {
        WS_XML_DOUBLE_TEXT *text_double = (WS_XML_DOUBLE_TEXT *)text;
        if ((hr = write_grow_buffer(writer, 1 + sizeof(text_double->value))) != S_OK)
            return hr;
        write_char(writer, type);
        write_bytes(writer, (const BYTE *)&text_double->value, sizeof(text_double->value));
        return S_OK;
    ...
    {
        WS_XML_GUID_TEXT *text_guid = (WS_XML_GUID_TEXT *)text;
        if ((hr = write_grow_buffer(writer, 1 + sizeof(text_guid->value))) != S_OK)
            return hr;
        write_char(writer, type);
        write_bytes(writer, (const BYTE *)&text_guid->value, sizeof(text_guid->value));
        return S_OK;
    ...
    {
        WS_XML_UNIQUE_ID_TEXT *text_unique_id = (WS_XML_UNIQUE_ID_TEXT *)text;
        if ((hr = write_grow_buffer(writer, 1 + sizeof(text_unique_id->value))) != S_OK)
            return hr;
        write_char(writer, type);
        write_bytes(writer, (const BYTE *)&text_unique_id->value, sizeof(text_unique_id->value));
        return S_OK;
...
{
    if (!writer->current->parent)
        return WS_E_INVALID_FORMAT;

    switch (writer->output_enc)
    {
    case WS_XML_WRITER_ENCODING_TYPE_TEXT:
        return write_text_text(writer, text, offset);
    case WS_XML_WRITER_ENCODING_TYPE_BINARY:
        return write_text_bin(writer, text, offset);
    default:
        ERR("unhandled encoding %u\n", writer->output_enc);
        return WS_E_NOT_SUPPORTED;
    }
...
static HRESULT write_text_node(struct writer *writer, const WS_XML_TEXT *text)
{
    ...
    if (node_type(writer->current) != WS_XML_NODE_TYPE_TEXT)
    {
        if ((hr = write_add_text_node(writer, text)) != S_OK)
            return hr;
    ...
    else
    {
        ...
        case WS_XML_WRITER_ENCODING_TYPE_TEXT:
        {
            ...
            if ((hr = text_to_utf8text(text, old, &offset, &new)) != S_OK)
                return hr;
            heap_free(old);
            node->text = &new->text;
            break;
        ...
        case WS_XML_WRITER_ENCODING_TYPE_BINARY:
        {
            ...
            if ((hr = text_to_text(text, old, &offset, &new)) != S_OK)
                return hr;
            heap_free(old);
            node->text = new;
            break;
    ...
    if ((hr = write_text(writer, node->text, offset)) != S_OK)
        return hr;
...
HRESULT WINAPI WsWriteText(WS_XML_WRITER *handle, const WS_XML_TEXT *text, WS_ERROR *error)
{
    ...
    if (writer->state == WRITER_STATE_STARTATTRIBUTE)
        hr = write_set_attribute_value(writer, text);
    else
        hr = write_text_node(writer, text);
...
HRESULT WINAPI WsWriteBytes(WS_XML_WRITER *handle, const void *bytes, ULONG count, WS_ERROR *error)
{
    ...
    if (writer->state == WRITER_STATE_STARTATTRIBUTE)
        hr = write_set_attribute_value(writer, &base64.text);
    else
        hr = write_text_node(writer, &base64.text);
...
HRESULT WINAPI WsWriteChars(WS_XML_WRITER *handle, const WCHAR *chars, ULONG count, WS_ERROR *error)
{
    ...
    if (writer->state == WRITER_STATE_STARTATTRIBUTE)
        hr = write_set_attribute_value(writer, &utf16.text);
    else
        hr = write_text_node(writer, &utf16.text);
...
HRESULT WINAPI WsWriteCharsUtf8(WS_XML_WRITER *handle, const BYTE *bytes, ULONG count, WS_ERROR *error)
{
    ...
    if (writer->state == WRITER_STATE_STARTATTRIBUTE)
        hr = write_set_attribute_value(writer, &utf8.text);
    else
        hr = write_text_node(writer, &utf8.text);
...
static HRESULT write_add_nil_attribute(struct writer *writer)
{
    ...
    if ((hr = write_add_attribute(writer, &prefix, &localname, &ns, FALSE)) != S_OK)
        return hr;
    if ((hr = write_set_attribute_value(writer, &value.text)) != S_OK)
        return hr;
                             ...
                             const void **ptr)
{
    ...
    case WS_WRITE_REQUIRED_POINTER:
        if (size != sizeof(const void *) || !(*ptr = *(const void **)value))
            return E_INVALIDARG;
        return S_OK;
                               ...
                               const BOOL *value, ULONG size)
{
    ...
    if (!option || option == WS_WRITE_NILLABLE_VALUE)
        return E_INVALIDARG;
    if ((hr = get_value_ptr(option, value, size, sizeof(BOOL), (const void **)&ptr)) != S_OK)
        return hr;
    if (option == WS_WRITE_NILLABLE_POINTER && !ptr)
                               ...
                               const BOOL *value, ULONG size)
{
    ...
    if (!option || option == WS_WRITE_NILLABLE_VALUE)
        return E_INVALIDARG;
    if ((hr = get_value_ptr(option, value, size, sizeof(INT8), (const void **)&ptr)) != S_OK)
        return hr;
    if (option == WS_WRITE_NILLABLE_POINTER && !ptr)
        return write_add_nil_attribute(writer);
                                ...
                                const BOOL *value, ULONG size)
{
    ...
    if (!option || option == WS_WRITE_NILLABLE_VALUE)
        return E_INVALIDARG;
    if ((hr = get_value_ptr(option, value, size, sizeof(INT16), (const void **)&ptr)) != S_OK)
        return hr;
    if (option == WS_WRITE_NILLABLE_POINTER && !ptr)
        return write_add_nil_attribute(writer);
                                ...
                                const void *value, ULONG size)
{
    ...
    if (!option || option == WS_WRITE_NILLABLE_VALUE)
        return E_INVALIDARG;
    if ((hr = get_value_ptr(option, value, size, sizeof(INT32), (const void **)&ptr)) != S_OK)
        return hr;
    if (option == WS_WRITE_NILLABLE_POINTER && !ptr)
        return write_add_nil_attribute(writer);
                                ...
                                const void *value, ULONG size)
{
    ...
    if (!option || option == WS_WRITE_NILLABLE_VALUE)
        return E_INVALIDARG;
    if ((hr = get_value_ptr(option, value, size, sizeof(INT64), (const void **)&ptr)) != S_OK)
        return hr;
    if (option == WS_WRITE_NILLABLE_POINTER && !ptr)
        return write_add_nil_attribute(writer);
                                ...
                                const void *value, ULONG size)
{
    ...
    if (!option || option == WS_WRITE_NILLABLE_VALUE)
        return E_INVALIDARG;
    if ((hr = get_value_ptr(option, value, size, sizeof(UINT8), (const void **)&ptr)) != S_OK)
        return hr;
    if (option == WS_WRITE_NILLABLE_POINTER && !ptr)
        return write_add_nil_attribute(writer);
                                 ...
                                 const void *value, ULONG size)
{
    ...
    if (!option || option == WS_WRITE_NILLABLE_VALUE)
        return E_INVALIDARG;
    if ((hr = get_value_ptr(option, value, size, sizeof(UINT16), (const void **)&ptr)) != S_OK)
        return hr;
    if (option == WS_WRITE_NILLABLE_POINTER && !ptr)
        return write_add_nil_attribute(writer);
                                 ...
                                 const void *value, ULONG size)
{
    ...
    if (!option || option == WS_WRITE_NILLABLE_VALUE)
        return E_INVALIDARG;
    if ((hr = get_value_ptr(option, value, size, sizeof(UINT32), (const void **)&ptr)) != S_OK)
        return hr;
    if (option == WS_WRITE_NILLABLE_POINTER && !ptr)
        return write_add_nil_attribute(writer);
                                 ...
                                 const void *value, ULONG size)
{
    ...
    if (!option || option == WS_WRITE_NILLABLE_VALUE)
        return E_INVALIDARG;
    if ((hr = get_value_ptr(option, value, size, sizeof(UINT64), (const void **)&ptr)) != S_OK)
        return hr;
    if (option == WS_WRITE_NILLABLE_POINTER && !ptr)
        return write_add_nil_attribute(writer);
                                 ...
                                 const void *value, ULONG size)
{
    ...
    if (!option || option == WS_WRITE_NILLABLE_VALUE)
        return E_INVALIDARG;
    if ((hr = get_value_ptr(option, value, size, sizeof(double), (const void **)&ptr)) != S_OK)
        return hr;
    if (option == WS_WRITE_NILLABLE_POINTER && !ptr)
        return write_add_nil_attribute(writer);
                                   ...
                                   const void *value, ULONG size)
{
    ...
    if (!option || option == WS_WRITE_NILLABLE_VALUE)
        return E_INVALIDARG;
    if ((hr = get_value_ptr(option, value, size, sizeof(WS_DATETIME), (const void **)&ptr)) != S_OK)
        return hr;
    if (option == WS_WRITE_NILLABLE_POINTER && !ptr)
        return write_add_nil_attribute(writer);
    if (ptr->ticks > TICKS_MAX || ptr->format > WS_DATETIME_FORMAT_NONE)
        return WS_E_INVALID_FORMAT;
                               ...
                               const void *value, ULONG size)
{
    ...
    if (!option || option == WS_WRITE_NILLABLE_VALUE)
        return E_INVALIDARG;
    if ((hr = get_value_ptr(option, value, size, sizeof(GUID), (const void **)&ptr)) != S_OK)
        return hr;
    if (option == WS_WRITE_NILLABLE_POINTER && !ptr)
        return write_add_nil_attribute(writer);
                                    ...
                                    const void *value, ULONG size)
{
    ...
    if (!option || option == WS_WRITE_NILLABLE_VALUE)
        return E_INVALIDARG;
    if ((hr = get_value_ptr(option, value, size, sizeof(*ptr), (const void **)&ptr)) != S_OK)
        return hr;
    if (option == WS_WRITE_NILLABLE_POINTER && !ptr)
        return write_add_nil_attribute(writer);
                                 ...
                                 const void *value, ULONG size)
{
    ...
    if (!option)
        return E_INVALIDARG;
    if ((hr = get_value_ptr(option, value, size, sizeof(WS_STRING), (const void **)&ptr)) != S_OK)
        return hr;
    if (option == WS_WRITE_NILLABLE_POINTER && !ptr)
        return write_add_nil_attribute(writer);
    if (!ptr->length)
                              ...
                              const void *value, ULONG size)
{
    ...
    if (!option || option == WS_WRITE_REQUIRED_VALUE || option == WS_WRITE_NILLABLE_VALUE)
        return E_INVALIDARG;
    if ((hr = get_value_ptr(option, value, size, 0, (const void **)&ptr)) != S_OK)
        return hr;
    if (option == WS_WRITE_NILLABLE_POINTER && !ptr)
        return write_add_nil_attribute(writer);
    if (!(len = strlenW(ptr)))
                                ...
                                const void *value, ULONG size)
{
    ...
    if (!option)
        return E_INVALIDARG;
    if ((hr = get_value_ptr(option, value, size, sizeof(WS_BYTES), (const void **)&ptr)) != S_OK)
        return hr;
    if ((option == WS_WRITE_NILLABLE_VALUE && is_nil_value(value, size)) ||
        (option == WS_WRITE_NILLABLE_POINTER && !ptr))
        return write_add_nil_attribute(writer);
    if (!ptr->length)
        return S_OK;
                                     ...
                                     const void *value, ULONG size)
{
    ...
    if (!option)
        return E_INVALIDARG;
    if ((hr = get_value_ptr(option, value, size, sizeof(WS_XML_STRING), (const void **)&ptr)) != S_OK)
        return hr;
    if (option == WS_WRITE_NILLABLE_POINTER && !ptr)
        return write_add_nil_attribute(writer);
    if (option == WS_WRITE_NILLABLE_VALUE && is_nil_value(value, size))
...
static HRESULT find_prefix(struct writer *writer, const WS_XML_STRING *ns, const WS_XML_STRING **prefix)
{
    ...
    for (node = writer->current; node_type(node) == WS_XML_NODE_TYPE_ELEMENT; node = node->parent)
    {
        ...
        for (i = 0; i < elem->attributeCount; i++)
        {
            if (!elem->attributes[i]->isXmlNs)
                continue;
            if (WsXmlStringEquals(elem->attributes[i]->ns, ns, NULL) != S_OK)
                continue;
                                ...
                                const void *value, ULONG size)
{
    ...
    if (((hr = find_prefix(writer, &ptr->ns, &prefix)) != S_OK))
        return hr;
...
static WS_WRITE_OPTION get_field_write_option(WS_TYPE type, ULONG options)
{
    ...
    {
        if (options & (WS_FIELD_OPTIONAL | WS_FIELD_NILLABLE))
                                ...
                                const void *value, ULONG size)
{
    ...
    if ((hr = get_value_ptr(option, value, size, desc->size, &ptr)) != S_OK)
        return hr;

    if (size < sizeof(enum_value))
        return E_INVALIDARG;
                                ...
                                ULONG count)
{
    ...
    if (!(option = get_field_write_option(desc->type, desc->options)))
        return E_INVALIDARG;
    ...
    for (i = 0; i < count; i++)
    {
        ...
        {
            if ((hr = write_element_node(writer, NULL, desc->itemLocalName, desc->itemNs)) != S_OK)
                return hr;
            if ((hr = write_type(writer, WS_ELEMENT_TYPE_MAPPING, desc->type, desc->typeDescription, option,
                                 buf + offset, size)) != S_OK)
                return hr;
            if ((hr = write_endelement_node(writer)) != S_OK)
                return hr;
        }
        offset += size;
    }
                                ...
                                ULONG offset)
{
    ...
    if (desc->type == WS_WSZ_TYPE)
        field_options |= WS_FIELD_POINTER;
    ...
    {
        if (field_options & WS_FIELD_OPTIONAL)
            return S_OK;
        ...
        {
            if (field_options & WS_FIELD_POINTER)
                option = WS_WRITE_REQUIRED_POINTER;
            else
                option = WS_WRITE_REQUIRED_VALUE;
        }
    }
    else
    {
    ...
    case WS_ELEMENT_FIELD_MAPPING:
        if ((hr = write_element_node(writer, NULL, desc->localName, desc->ns)) != S_OK)
            return hr;
                                 ...
                                 const void *value, ULONG size)
{
    ...
    for (i = 0; i < desc->fieldCount; i++)
    {
        ...
        if ((hr = write_type_field(writer, desc->fields[i], ptr, offset)) != S_OK)
            return hr;
    }
                              ...
                              WS_ERROR *error)
{
    ...
    if ((hr = write_element_node(writer, NULL, desc->elementLocalName, desc->elementNs)) != S_OK)
        goto done;

    if ((hr = write_type(writer, WS_ANY_ELEMENT_TYPE_MAPPING, desc->type, desc->typeDescription,
                         option, value, size)) != S_OK)
        goto done;
                            ...
                            ULONG size, WS_ERROR *error)
{
    ...
    if (!writer || !value || (type = map_value_type(value_type)) == ~0u)
        return E_INVALIDARG;
    ...
    if (hr == S_OK)
        hr = write_type(writer, mapping, type, NULL, WS_WRITE_REQUIRED_VALUE, value, size);
                            ...
                            ULONG count, WS_ERROR *error)
{
    ...
    for (i = offset; i < count; i++)
    {
        ...
        if ((hr = write_element_node(writer, NULL, localname, ns)) != S_OK)
            goto done;
...
HRESULT WINAPI WsWriteXmlBuffer(WS_XML_WRITER *handle, WS_XML_BUFFER *buffer, WS_ERROR *error)
{
    ...
    if ((hr = write_flush(writer)) != S_OK)
        goto done;
    if ((hr = write_grow_buffer(writer, xmlbuf->bytes.length)) != S_OK)
        goto done;
                                       ...
                                       WS_HEAP *heap, void **bytes, ULONG *size, WS_ERROR *error)
{
    ...
    if (!(buf = ws_alloc(heap, xmlbuf->bytes.length)))
        hr = WS_E_QUOTA_EXCEEDED;
    else
    {
                                    ...
                                    const WS_XML_STRING *localname, const WS_XML_STRING *ns)
{
    ...
    if ((hr = write_flush(writer)) != S_OK)
        return hr;
    if (!prefix && ((hr = find_prefix(writer, ns, &prefix)) != S_OK))
        return hr;
    ...
    if ((hr = write_add_text_node(writer, &qname.text)) != S_OK)
        return hr;
...
static HRESULT write_add_comment_node(struct writer *writer, const WS_XML_STRING *value)
{
    ...
    if (!(parent = find_parent(writer)))
        return WS_E_INVALID_FORMAT;
    if (!(node = alloc_node(WS_XML_NODE_TYPE_COMMENT)))
        return E_OUTOFMEMORY;
...
static HRESULT write_comment_text(struct writer *writer)
{
    ...
    if ((hr = write_grow_buffer(writer, comment->value.length + 7)) != S_OK)
        return hr;
...
static HRESULT write_comment_node(struct writer *writer, const WS_XML_STRING *value)
{
    ...
    if ((hr = write_flush(writer)) != S_OK)
        return hr;
    if ((hr = write_add_comment_node(writer, value)) != S_OK)
        return hr;
    if ((hr = write_comment(writer)) != S_OK)
        return hr;
...
static HRESULT write_set_attributes(struct writer *writer, WS_XML_ATTRIBUTE **attrs, ULONG count)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if (attrs[i]->isXmlNs)
        {
            if ((hr = add_namespace_attribute(writer, prefix, ns, single)) != S_OK)
                return hr;
        }
        else
        {
            if ((hr = write_add_attribute(writer, prefix, localname, ns, single)) != S_OK)
                return hr;
            if ((hr = write_set_attribute_value(writer, attrs[i]->value)) != S_OK)
                return hr;
        }
    }
    return S_OK;
...
static HRESULT write_node(struct writer *writer, const WS_XML_NODE *node)
{
    ...
    case WS_XML_NODE_TYPE_ELEMENT:
    {
        ...
        if ((hr = write_element_node(writer, elem->prefix, elem->localName, elem->ns)) != S_OK)
            return hr;
...
static HRESULT write_tree_node(struct writer *writer)
{
    ...
    case WS_XML_NODE_TYPE_TEXT:
        if (writer->state == WRITER_STATE_STARTELEMENT && (hr = write_endstartelement(writer)) != S_OK)
            return hr;
        if ((hr = write_text(writer, ((const WS_XML_TEXT_NODE *)writer->current)->text, 0)) != S_OK)
            return hr;
        writer->state = WRITER_STATE_TEXT;
        return S_OK;

    case WS_XML_NODE_TYPE_END_ELEMENT:
        if ((hr = write_close_element(writer, writer->current->parent)) != S_OK)
            return hr;
        writer->state = WRITER_STATE_ENDELEMENT;
        return S_OK;
...
static HRESULT write_tree(struct writer *writer)
{
    ...
    if ((hr = write_tree_node(writer)) != S_OK)
        return hr;
    for (;;)
    {
        if (node_type(writer->current) == WS_XML_NODE_TYPE_EOF)
            break;
...
HRESULT WINAPI WsCopyNode(WS_XML_WRITER *handle, WS_XML_READER *reader, WS_ERROR *error)
{
    ...
    if ((hr = copy_node(reader, writer->output_enc, &node)) != S_OK)
        goto done;
    current = writer->current;
    write_insert_node(writer, parent, node);

    write_rewind(writer);
    if ((hr = write_tree(writer)) != S_OK)
        goto done;
                           ...
                           const WS_PARAMETER_DESCRIPTION *params, ULONG count, const void **args)
{
    ...
    if (desc->type != WS_STRUCT_TYPE || !(desc_struct = desc->typeDescription))
        return E_INVALIDARG;
    ...
    for (i = 0; i < count; i++)
    {
        if (params[i].inputMessageIndex == INVALID_PARAMETER_INDEX)
            continue;
        ...
        if ((hr = get_param_desc(desc_struct, params[i].inputMessageIndex, &desc_field)) != S_OK)
            goto done;
        if (params[i].parameterType == WS_PARAMETER_TYPE_NORMAL)
        {
            if ((hr = write_param(writer, desc_field, args[i])) != S_OK)
                goto done;
        }
        else if (params[i].parameterType == WS_PARAMETER_TYPE_ARRAY)
        {
            ...
            if ((hr = write_param_array(writer, desc_field, ptr, len)) != S_OK)
                goto done;
        }
    }
