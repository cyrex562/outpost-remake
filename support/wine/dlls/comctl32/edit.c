	...
	do
	{
		...
		if (current_line->net_length)
		{
			...
			if (current_line->ssa)
			{
				...
				current_line->width = es->char_width * current_line->net_length;
		}
		else
 ...
 */
		...
		EDIT_InvalidateText(es, start, old_end);
...
static LRESULT EDIT_WM_VScroll(EDITSTATE *es, INT action, INT pos)
{
	...
	case SB_PAGEUP:
	case SB_PAGEDOWN:
		TRACE("action %d (%s)\n", action, (action == SB_LINEUP ? "SB_LINEUP" : (action == SB_LINEDOWN ? "SB_LINEDOWN" : (action == SB_PAGEUP ? "SB_PAGEUP" : "SB_PAGEDOWN"))));
...
static LRESULT CALLBACK EDIT_WindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case EM_GETSEL:
		result = EDIT_EM_GetSel(es, wParam, lParam);
		break;
	...
	case EM_POSFROMCHAR:
		if ((INT)wParam >= get_text_length(es))
			result = -1;
		else
