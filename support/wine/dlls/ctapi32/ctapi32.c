...
static IS8 (*pCT_init)(Ictn: u16, Ipn: u16) = NULL;
static IS8 (*pCT_data)(Ictn: u16, IU8 *dad, IU8 *sad, Ilenc: u16, IU8 *command,
	IU16 *lenr, IU8 *response) = NULL;
static IS8 (*pCT_close)(Ictn: u16) = NULL;
...
IS8 WINAPI WIN_CT_init(Ictn: u16, Ipn: u16)
{
	if (!pCT_init)
		return ERR_HOST;
...
IS8 WINAPI WIN_CT_data(Ictn: u16, IU8 *dad, IU8 *sad, Ilenc: u16, IU8 *command, IU16 *lenr, IU8 *response)
{
	if (!pCT_data)
		return ERR_HOST;
...
IS8 WINAPI WIN_CT_close(Ictn: u16)
{
	if (!pCT_close)
		return ERR_HOST;
