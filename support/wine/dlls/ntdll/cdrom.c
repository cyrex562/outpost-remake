...
#define MSF_OF_FRAME(m, fr)                 \
    {                                       \
        int f = (fr);                       \
        ((UCHAR *)&(m))[2] = f % CD_FRAMES; \
        f /= CD_FRAMES;                     \
        ((UCHAR *)&(m))[1] = f % CD_SECS;   \
        ((UCHAR *)&(m))[0] = f / CD_SECS;   \
    }
...
static RTL_CRITICAL_SECTION_DEBUG critsect_debug =
    {
        0, 0, &cache_section, {&critsect_debug.ProcessLocksList, &critsect_debug.ProcessLocksList}, 0, 0, {(DWORD_PTR)(__FILE__ ": cache_section")}};
...
static NTSTATUS get_parent_device(int fd, char *name, size_t len)
{
    ...
    if (!(dict = IOServiceMatching(kIOMediaClass)))
...
static NTSTATUS CDROM_SyncCache(int dev, int fd)
{
    ...
    tsz = sizeof(toc->FirstTrack) + sizeof(toc->LastTrack) + sizeof(TRACK_DATA) * (toc->LastTrack - toc->FirstTrack + 2);
    toc->Length[0] = tsz >> 8;
    toc->Length[1] = tsz;
...
static NTSTATUS CDROM_GetDriveGeometry(int dev, int fd, DISK_GEOMETRY *dg)
{
    ...
    if ((ret = CDROM_ReadTOC(dev, fd, &toc)) != 0)
        return ret;

    fsize = FRAME_OF_TOC(toc, toc.LastTrack + 1) - FRAME_OF_TOC(toc, 1); /* Total size in frames */
...
#elif defined(__APPLE__)
    if (doEject)
        return CDROM_GetStatusCode(ioctl(fd, DKIOCEJECT, NULL));
    else
        return STATUS_NOT_SUPPORTED;
#else
    FIXME("not supported on this O/S\n");
                                   ...
                                   SUB_Q_CHANNEL_DATA *data)
{
    ...
    case IOCTL_CDROM_MEDIA_CATALOG:
        data->MediaCatalog.FormatCode = IOCTL_CDROM_MEDIA_CATALOG;
        {
            ...
            if ((io = ioctl(fd, CDROM_GET_MCN, &mcn)) == -1)
                goto end;
    ...
    {
    case IOCTL_CDROM_MEDIA_CATALOG:
        if ((io = ioctl(fd, DKIOCCDREADMCN, &ioc.mcn)) == -1)
            break;
    ...
    case IOCTL_CDROM_TRACK_ISRC:
        ...
        if ((io = ioctl(fd, DKIOCCDREADISRC, &ioc.isrc)) == -1)
            break;
...
static NTSTATUS CDROM_SeekAudioMSF(int dev, int fd, const CDROM_SEEK_AUDIO_MSF *audio_msf)
{
    ...
    if ((io = CDROM_ReadTOC(dev, fd, &toc)) != 0)
        return io;
...
static NTSTATUS DVD_StartSession(int fd, const DVD_SESSION_ID *sid_in, PDVD_SESSION_ID sid_out)
{
    ...
    if (sid_in)
        auth_info.lsa.agid = *(const int *)sid_in; /* ?*/
    ...
    if (ret == STATUS_SUCCESS)
    {
        ...
        case DvdTitleKey:
            ...
            if (desc.title.CPM)
            {
                ...
                if (desc.title.CP_SEC)
                    key->KeyFlags |= DVD_SECTOR_PROTECTED;
                               ...
                               LPVOID lpOutBuffer, DWORD nOutBufferSize)
{
    ...
    if ((status = server_get_unix_fd(hDevice, 0, &fd, &needs_close, NULL, NULL)))
    {
        if (status == STATUS_BAD_DEVICE_TYPE)
            return status; /* no associated fd */
        goto error;
    }
...
#ifdef __APPLE__
    {
         ...
         */
        if ((status = get_parent_device(fd, name, sizeof(name))))
            goto error;
    ...
    case IOCTL_STORAGE_EJECTION_CONTROL:
        ...
        sz = 0;
        CDROM_ClearCacheEntry(dev);
        if (lpOutBuffer != NULL || nOutBufferSize != 0)
            status = STATUS_INVALID_PARAMETER;
        else if (nInBufferSize < sizeof(PREVENT_MEDIA_REMOVAL))
            status = STATUS_BUFFER_TOO_SMALL;
        else
            status = CDROM_ControlEjection(fd, lpInBuffer);
        break;
    ...
    case IOCTL_STORAGE_GET_MEDIA_TYPES_EX:
        ...
        if (lpInBuffer != NULL || nInBufferSize != 0)
            status = STATUS_INVALID_PARAMETER;
        else if (nOutBufferSize < sz)
            status = STATUS_BUFFER_TOO_SMALL;
        else
            status = CDROM_GetMediaType(dev, lpOutBuffer);
        break;
    ...
    case IOCTL_STORAGE_GET_DEVICE_NUMBER:
        ...
        if (lpInBuffer != NULL || nInBufferSize != 0)
            ...
            status = CDROM_GetDeviceNumber(dev, lpOutBuffer);
        break;
    ...
    case IOCTL_CDROM_GET_CONTROL:
        ...
        if (lpInBuffer != NULL || nInBufferSize != 0)
            ...
            status = CDROM_GetControl(dev, fd, lpOutBuffer);
        break;
    ...
    case IOCTL_CDROM_GET_DRIVE_GEOMETRY:
        ...
        if (lpInBuffer != NULL || nInBufferSize != 0)
            ...
            status = CDROM_GetDriveGeometry(dev, fd, lpOutBuffer);
        break;
    ...
    case IOCTL_CDROM_DISK_TYPE:
        ...
        if (lpInBuffer != NULL || nInBufferSize != 0)
            ...
            status = CDROM_GetDiskData(dev, fd, lpOutBuffer);
        break;
    ...
    case IOCTL_CDROM_READ_Q_CHANNEL:
        ...
        if (lpInBuffer == NULL || nInBufferSize < sizeof(CDROM_SUB_Q_DATA_FORMAT))
            ...
            status = CDROM_ReadQChannel(dev, fd, lpInBuffer, lpOutBuffer);
        break;
    ...
    case IOCTL_CDROM_READ_TOC:
        ...
        if (lpInBuffer != NULL || nInBufferSize != 0)
            ...
            status = CDROM_ReadTOC(dev, fd, lpOutBuffer);
        break;
    ...
    case IOCTL_CDROM_PAUSE_AUDIO:
        ...
        if (lpInBuffer != NULL || nInBufferSize != 0 || lpOutBuffer != NULL || nOutBufferSize != 0)
            ...
            status = CDROM_PauseAudio(fd);
        break;
    case IOCTL_CDROM_PLAY_AUDIO_MSF:
        sz = 0;
        if (lpOutBuffer != NULL || nOutBufferSize != 0)
            status = STATUS_INVALID_PARAMETER;
        else if (nInBufferSize < sizeof(CDROM_PLAY_AUDIO_MSF))
            status = STATUS_BUFFER_TOO_SMALL;
        else
            status = CDROM_PlayAudioMSF(fd, lpInBuffer);
        break;
    case IOCTL_CDROM_RESUME_AUDIO:
        ...
        if (lpInBuffer != NULL || nInBufferSize != 0 || lpOutBuffer != NULL || nOutBufferSize != 0)
            ...
            status = CDROM_ResumeAudio(fd);
        break;
    case IOCTL_CDROM_SEEK_AUDIO_MSF:
        sz = 0;
        if (lpOutBuffer != NULL || nOutBufferSize != 0)
            status = STATUS_INVALID_PARAMETER;
        else if (nInBufferSize < sizeof(CDROM_SEEK_AUDIO_MSF))
            status = STATUS_BUFFER_TOO_SMALL;
        else
            status = CDROM_SeekAudioMSF(dev, fd, lpInBuffer);
        break;
    ...
    case IOCTL_CDROM_GET_VOLUME:
        ...
        if (lpInBuffer != NULL || nInBufferSize != 0)
            ...
            status = CDROM_GetVolume(fd, lpOutBuffer);
        break;
    case IOCTL_CDROM_SET_VOLUME:
        ...
        if (lpInBuffer == NULL || nInBufferSize < sizeof(VOLUME_CONTROL) || lpOutBuffer != NULL)
            ...
            status = CDROM_SetVolume(fd, lpInBuffer);
        break;
    case IOCTL_CDROM_RAW_READ:
        sz = 0;
        if (nInBufferSize < sizeof(RAW_READ_INFO))
            status = STATUS_INVALID_PARAMETER;
        else if (lpOutBuffer == NULL)
            status = STATUS_BUFFER_TOO_SMALL;
        else
            status = CDROM_RawRead(fd, lpInBuffer, lpOutBuffer,
                                   nOutBufferSize, &sz);
        break;
    ...
    case IOCTL_SCSI_PASS_THROUGH_DIRECT:
        ...
        if (lpOutBuffer == NULL)
            status = STATUS_INVALID_PARAMETER;
        else if (nOutBufferSize < sizeof(SCSI_PASS_THROUGH_DIRECT))
            status = STATUS_BUFFER_TOO_SMALL;
        else
            status = CDROM_ScsiPassThroughDirect(fd, lpOutBuffer);
        break;
    case IOCTL_SCSI_PASS_THROUGH:
        sz = sizeof(SCSI_PASS_THROUGH);
        if (lpOutBuffer == NULL)
            status = STATUS_INVALID_PARAMETER;
        else if (nOutBufferSize < sizeof(SCSI_PASS_THROUGH))
            status = STATUS_BUFFER_TOO_SMALL;
        else
            status = CDROM_ScsiPassThrough(fd, lpOutBuffer);
        break;
    case IOCTL_SCSI_GET_CAPABILITIES:
        sz = sizeof(IO_SCSI_CAPABILITIES);
        if (lpOutBuffer == NULL)
            status = STATUS_INVALID_PARAMETER;
        else if (nOutBufferSize < sizeof(IO_SCSI_CAPABILITIES))
            status = STATUS_BUFFER_TOO_SMALL;
        else
            status = CDROM_ScsiGetCaps(fd, lpOutBuffer);
        break;
    ...
    case IOCTL_DVD_END_SESSION:
        ...
        if ((lpInBuffer == NULL) || (nInBufferSize < sz))
            ...
            status = DVD_EndSession(fd, lpInBuffer);
        break;
    ...
    case IOCTL_DVD_READ_KEY:
        if (!lpInBuffer ||
            (((PDVD_COPY_PROTECT_KEY)lpInBuffer)->KeyLength != nInBufferSize))
            status = STATUS_INVALID_PARAMETER;
        else if (lpInBuffer != lpOutBuffer)
            status = STATUS_BUFFER_TOO_SMALL;
        else
        {
    ...
    case IOCTL_DVD_READ_STRUCTURE:
        ...
        if (lpInBuffer == NULL || nInBufferSize != sizeof(DVD_READ_STRUCTURE))
            status = STATUS_INVALID_PARAMETER;
        else if (nOutBufferSize < sz || !lpOutBuffer)
            status = STATUS_BUFFER_TOO_SMALL;
        else
        {
