...
static int free_icon_handle(HICON16 handle)
{
    ...
    if ((icon32 = store_icon_32(handle, 0)))
        DestroyIcon(icon32);
...
HICON16 get_icon_16(HICON icon)
{
    ...
    if (!ret)
    {
        ...
        if (!(and_bits = HeapAlloc(GetProcessHeap(), 0, and_size)))
            goto done;
        ...
        if (info.hbmColor)
        {
            ...
            if (!(xor_bits = HeapAlloc(GetProcessHeap(), 0, xor_size)))
                goto done;
        ...
        if (!info.fIcon)
        {
            ...
            cinfo.ptHotSpot.x = cinfo.ptHotSpot.y = ICON_HOTSPOT;
...
static HICON16 find_shared_icon(HINSTANCE16 inst, HRSRC16 rsrc)
{
    ...
    {
        if (cache->inst != inst || cache->rsrc != rsrc)
            continue;
        cache->count++;
        return cache->icon;
    }
...
INT16 WINAPI FillRect16(HDC16 hdc, const RECT16 *rect, HBRUSH16 hbrush)
{
    ...
    if (!(prevBrush = SelectObject(HDC_32(hdc), HBRUSH_32(hbrush))))
        return 0;
...
INT16 WINAPI DrawText16(HDC16 hdc, LPCSTR str, INT16 count, LPRECT16 rect, UINT16 flags)
{
    ...
    if (rect)
    {
        ...
        ret = DrawTextA(HDC_32(hdc), str, count, NULL, flags);
    return ret;
}
...
HANDLE16 WINAPI SetClipboardData16(UINT16 format, HANDLE16 data16)
{
    ...
    case CF_METAFILEPICT:
    {
        ...
        if (pict16)
        {
            if (!(data32 = GlobalAlloc(GMEM_MOVEABLE, sizeof(*pict32))))
                return 0;
    ...
    default:
        ...
        else
        {
            ...
            if (ptr16)
            {
                if (!(data32 = GlobalAlloc(GMEM_MOVEABLE, size)))
                    return 0;
    ...
    if (!SetClipboardData(format, data32))
        return 0;
    return data16;
}
...
HANDLE16 WINAPI GetClipboardData16(UINT16 format)
{
    ...
    case CF_METAFILEPICT:
    {
        ...
        if (pict32)
        {
            if (!(data16 = GlobalAlloc16(GMEM_MOVEABLE, sizeof(*pict16))))
                return 0;
    ...
    default:
        ...
        else
        {
            ...
            if (ptr32)
            {
                ...
                if (!(data16 = GlobalAlloc16(GMEM_MOVEABLE, size)))
                    return 0;
...
HMENhMenu:
    ...
    if (HIWORD(name) && name[0] == '#')
        name = ULongToPtr(atoi(name + 1));
    if (!name)
        return 0;

    instance = GetExePtr(instance);
    if (!(hRsrc = FindResource16(instance, name, (LPSTR)RT_MENU)))
        return 0;
    if (!(handle = LoadResource16(instance, hRsrc)))
        return 0;
...
{
    if (flags & MF_APPEND)
        return AppendMenu16(hMenu, flags & ~MF_APPEND, id, data);
    ...
    if (flags & MF_DELETE)
        return DeleteMenu16(hMenu, pos, flags & ~MF_DELETE);
    if (flags & MF_CHANGE)
        return ModifyMenu16(hMenu, pos, flags & ~MF_CHANGE, id, data);
    if (flags & MF_REMOVE)
        return RemoveMenu16(hMenu, flags & MF_BYPOSITION ? pos : id,
                            flags & ~MF_REMOVE);
    /* Default: MF_INSERT */
                           ...
                           INT16 cx, INT16 cy)
{
    ...
    if (!gsprc)
        return GrayStringA(HDC_32(hdc), HBRUSH_32(hbr), NULL,
 ...
 */
HMENU16 WINAPI LookupMenuHandle16(HMENhmenu
                                  : u16, INT16 id)
{
    FIXME("%04x %04x: stub\n", hmenu, id);
    return hmenu;
...
static LPCSTR parse_menu_resource(LPCSTR res, HMENU hMenu, BOOL oldFormat)
{
    ...
    do
    {
        ...
        if (flags & MF_POPUP)
        {
            ...
            if (!hSubMenu)
                return NULL;
            if (!(res = parse_menu_resource(res, hSubMenu, oldFormat)))
                return NULL;
 ...
 */
INT16 WINAPI GetMenuItemCount16(HMENhMenu
                                : u16)
{
...
void WINAPI keybd_event16(CONTEXT *context)
{
    ...
    if (HIBYTE(context->Eax) & 0x80)
        dwFlags |= KEYEVENTF_KEYUP;
    if (HIBYTE(context->Ebx) & 0x01)
 ...
 *		IsMenu    (USER.358)
 */
BOOL16 WINAPI IsMenu16(HMENhmenu
                       : u16)
{
                             ...
                             const RECT16 *src2)
{
    ...
    *dest = *src1;
    if (IntersectRect16(&tmp, src1, src2))
    {
        ...
        {
            if (tmp.left == dest->left)
                dest->left = tmp.right;
            else if (tmp.right == dest->right)
                dest->right = tmp.left;
        }
        else if ((tmp.left == dest->left) && (tmp.right == dest->right))
        {
            if (tmp.top == dest->top)
                dest->top = tmp.bottom;
            else if (tmp.bottom == dest->bottom)
                dest->bottom = tmp.top;
        }
    }
    return TRUE;
 ...
 */
BOOL16 WINAPI SetMenuContextHelpId16(HMENhMenu
                                     : u16, DWORD dwContextHelpID)
{
 ...
 */
DWORD WINAPI GetMenuContextHelpId16(HMENhMenu
                                    : u16)
{
...
HANDLE16 WINAPI LoadImage16(HINSTANCE16 hinst, LPCSTR name, UINT16 type, INT16 cx, INT16 cy, UINT16 flags)
{
    ...
    case IMAGE_BITMAP:
    {
        ...
        filename[0] = 0;
        if (!(hRsrc = FindResource16(hinst, name, (LPCSTR)RT_BITMAP)))
            return 0;
        if (!(handle = LoadResource16(hinst, hRsrc)))
            return 0;
        if (!(ptr = LockResource16(handle)))
            goto done;
        ...
        if (!GetTempPathW(MAX_PATH, path))
            goto done;
        if (!GetTempFileNameW(path, prefixW, 0, filename))
            goto done;
        ...
        if (file != INVALID_HANDLE_VALUE)
        {
            ...
            if (ok)
                ok = WriteFile(file, ptr, size, &written, NULL) && (written == size);
            CloseHandle(file);
            if (ok)
                ret = LoadImageW(0, filename, IMAGE_BITMAP, cx, cy, flags | LR_LOADFROMFILE);
        }
    done:
        if (filename[0])
    ...
    case IMAGE_CURSOR:
    {
        ...
        if (!(handle = LoadResource16(hinst, hRsrc)))
            return 0;
        if ((dir = LockResource16(handle)))
            id = LookupIconIdFromDirectory(dir, type == IMAGE_ICON);
        FreeResource16(handle);
        if (!id)
            return 0;
        ...
        if (hIcon && (flags & LR_SHARED))
            add_shared_icon(hinst, hRsrc, hGroupRsrc, hIcon);
        return hIcon;
    }
    default:
        return 0;
                           ...
                           INT16 desiredy, UINT16 flags)
{
    if (flags & LR_COPYFROMRESOURCE)
        FIXME("LR_COPYFROMRESOURCE not supported\n");
...
{
    UINT pos32 = (UINT)pos;
    if ((pos == (UINT16)-1) && (flags & MF_BYPOSITION))
        pos32 = (UINT)-1;
    ...
    if (flags & MF_BITMAP)
        data = (SEGPTR)HBITMAP_32(LOWORD(data));
 ...
 */
BOOL16 WINAPI SetMenuItemBitmaps16(HMENhMenu
                                   : u16, UINT16 nPos, UINT16 wFlags,
...
INT16 WINAPI wvsprintf16(LPSTR buffer, LPCSTR spec, VA_LIST16 args)
{
    ...
    while (*spec)
    {
        ...
        case WPR_STRING:
            ...
            if (IsBadReadPtr16(seg_str, 1))
                lpcstr_view = "";
            else
                lpcstr_view = MapSL(seg_str);
            if (!lpcstr_view)
                lpcstr_view = "(null)";
        ...
        case WPR_SIGNED:
            if (format.flags & WPRINTF_LONG)
                int_view = VA_ARG16(args, INT);
            else
                int_view = VA_ARG16(args, INT16);
            len = sprintf(number, "%d", int_view);
            break;
        case WPR_UNSIGNED:
            if (format.flags & WPRINTF_LONG)
                int_view = VA_ARG16(args, UINT);
            else
                int_view = VA_ARG16(args, UINT16);
        ...
        sign = 0;
        if (!(format.flags & WPRINTF_LEFTALIGN))
            for (i = format.precision; i < format.width; i++)
                *p++ = ' ';
        switch (format.type)
        {
        case WPR_CHAR:
        ...
        case WPR_SIGNED:
            ...
            /* fall through */
        case WPR_UNSIGNED:
            for (i = len; i < format.precision; i++)
                *p++ = '0';
            if (len > sign)
...
SEGPTR WINAPI AnsiUpper16(SEGPTR strOrChar)
{
    ...
    if (HIWORD(strOrChar))
    {
        ...
        return (SEGPTR)CharUpperA((LPSTR)strOrChar);
}
...
SEGPTR WINAPI AnsiLower16(SEGPTR strOrChar)
{
    ...
    if (HIWORD(strOrChar))
    {
        ...
        return (SEGPTR)CharLowerA((LPSTR)strOrChar);
}
                          ...
                          WPARAM16 wdata, INT16 x, INT16 y, INT16 cx, INT16 cy, UINT16 flags)
{
    ...
    if (opcode == DST_TEXT || opcode == DST_PREFIXTEXT)
    {
        ...
        if (!wdata)
            wdata = strlen(MapSL(ldata));
        if (!cx || !cy)
        {
            SIZE s;
            if (!GetTextExtentPoint32A(HDC_32(hdc), MapSL(ldata), wdata, &s))
                return FALSE;
            if (!cx)
                cx = s.cx;
            if (!cy)
                        ...
                        SEGPTR *lpXorBits, SEGPTR *lpAndBits)
{
    ...
    sizeAnd = info->nHeight * get_bitmap_width_bytes(info->nWidth, 1);
    if (lpAndBits)
        *lpAndBits = pInfo + sizeof(CURSORICONINFO);
    if (lpXorBits)
        *lpXorBits = pInfo + sizeof(CURSORICONINFO) + sizeAnd;
    if (lpLen)
        *lpLen = sizeof(CURSORICONINFO) + sizeAnd + sizeXor;
...
static BOOL DRAG_QueryUpdate16(HWND hQueryWnd, SEGPTR spDragInfo)
{
    ...
    {
        wParam = 0;
        if (DRAG_QueryUpdate16(child, spDragInfo))
            return TRUE;
    }
    else
    {
                          ...
                          HANDLE16 hOfStruct, WORD szList, HCURSOR16 hCursor)
{
    ...
    if ((hCursor32 = get_icon_32(hCursor)))
        SetCursor(hCursor32);
                                     ...
                                     LPVOID lpvParam, UINT16 fuWinIni)
{
    ...
    case SPI_GETICONTITLELOGFONT:
    {
        ...
        ret = SystemParametersInfoA(uAction, uParam, lpvParam ? &tmp : NULL, fuWinIni);
        if (ret && lpvParam)
            logfont_32_to_16(&tmp, (LPLOGFONT16)lpvParam);
        break;
    }
    ...
    LPDWORD args) /* [in] NOTE: va_list *args */
{
    ...
    if ((dwFlags & FORMAT_MESSAGE_FROM_SYSTEM) && (dwFlags & FORMAT_MESSAGE_FROM_HMODULE))
        return 0;
    if ((dwFlags & FORMAT_MESSAGE_FROM_STRING) && ((dwFlags & FORMAT_MESSAGE_FROM_SYSTEM) || (dwFlags & FORMAT_MESSAGE_FROM_HMODULE)))
        return 0;
...
#define ADD_TO_T(c)                                                                         \
    ...
    return (dwFlags & FORMAT_MESSAGE_ALLOCATE_BUFFER) ? strlen(allocstring) : strlen(lpBuffer);
}
#undef ADD_TO_T
 ...
 */
BOOL16 WINAPI CheckMenuRadioItem16(HMENhMenu
                                   : u16, UINT16 first, UINT16 last,
