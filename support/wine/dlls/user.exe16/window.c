...
INT16 WINAPI EnumProps16(HWND16 hwnd, PROPENUMPROC16 func)
{
    ...
    while (total)
    {
        if (!(list = HeapAlloc(GetProcessHeap(), 0, total * sizeof(*list))))
            break;
        count = 0;
...
HWND16 WINAPI CreateWindow16(LPCSTR className, LPCSTR windowName,
                             ...
                             INT16 height, HWND16 parent, HMENU16 hmenu,
...
void WINAPI SetScrollRange16(HWND16 hwnd, INT16 nBar, INT16 MinVal, INT16 MaxVal, BOOL16 redraw)
{
    ...
    if ((INT)MaxVal - (INT)MinVal > 0x7fff)
        MinVal = MaxVal = 0;
...
LONG WINAPI SetWindowLong16(HWND16 hwnd16, INT16 offset, LONG newval)
{
    ...
    if (is_winproc)
    {
        ...
        return SetWindowLongA(hwnd, offset, newval);
}
 ...
 *              SetMenu   (USER.158)
 */
BOOL16 WINAPI SetMenu16(HWND16 hwnd, HMENhMenu
                        : u16)
{
...
BOOL16 WINAPI GetUpdateRect16(HWND16 hwnd, LPRECT16 rect, BOOL16 erase)
{
    ...
    if (!rect)
        return GetUpdateRect(WIN_Handle32(hwnd), NULL, erase);
...
{
    if ((flag != GW_HWNDNEXT) && (flag != GW_HWNDPREV))
        return 0;
...
void WINAPI MapWindowPoints16(HWND16 hwndFrom, HWND16 hwndTo, LPPOINT16 lppt, UINT16 count)
{
    ...
    if (count > 8)
        ppt = HeapAlloc(GetProcessHeap(), 0, count * sizeof(*ppt));
    for (i = 0; i < count; i++)
    {
        ppt[i].x = lppt[i].x;
        ppt[i].y = lppt[i].y;
                        ...
                        HBRUSH16 hbrush, const RECT16 *rect)
{
    if (hbrush <= CTLCOLOR_STATIC)
    {
        ...
        if (!hbrush)
            hbrush = DefWindowProcW(parent, WM_CTLCOLORMSGBOX + hbrush,
                                    hdc, (LPARAM)hwnd32);
    }
    if (hbrush)
...
HBRUSH16 WINAPI GetControlBrush16(HWND16 hwnd, HDC16 hdc, UINT16 ctlType)
{
    ...
    if (!ret)
        ret = DefWindowProcW(parent, WM_CTLCOLORMSGBOX + ctlType,
                             hdc, (LPARAM)hwnd32);
    return ret;
}
...
BOOL16 WINAPI GetWindowPlacement16(HWND16 hwnd, WINDOWPLACEMENT16 *wp16)
{
    ...
    if (!GetWindowPlacement(WIN_Handle32(hwnd), &wpl))
        return FALSE;
...
BOOL16 WINAPI GetClassInfoEx16(HINSTANCE16 hInst16, SEGPTR name, WNDCLASSEX16 *wc)
{
    ...
    if (hInst16 == GetModuleHandle16("user"))
        hInstance = user32_module;
    else
        hInstance = HINSTANCE_32(GetExePtr(hInst16));
...
BOOL16 WINAPI UnregisterClass16(LPCSTR className, HINSTANCE16 hInstance)
{
    ...
    if (hInstance == GetModuleHandle16("user"))
        hInstance = 0;
    else
...
UINT16 WINAPI GetInternalWindowPos16(HWND16 hwnd, LPRECT16 rectWnd, LPPOINT16 ptIcon)
{
    ...
    if (!GetWindowPlacement16(hwnd, &wndpl))
        return 0;
    if (rectWnd)
        *rectWnd = wndpl.rcNormalPosition;
    if (ptIcon)
        *ptIcon = wndpl.ptMinPosition;
 ...
 */
BOOL16 WINAPI GetMenuItemRect16(HWND16 hwnd, HMENhMenu
                                : u16, UINT16 uItem,
                                  LPRECT16 rect)
{
    RECT r32;
