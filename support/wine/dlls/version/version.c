...
static int read_xx_header(HFILE lzfd)
{
    ...
    {
        if (!memcmp(&mzh, "\177ELF", 4))
            return 1; /* ELF */
        if (*&mzh == 0xfeedface || *&mzh == 0xcefaedfe)
            return 1; /* Mach-O */
        return 0;
    }
...
static BOOL find_ne_resource(HFILE lzfd, DWORD *resLen, DWORD *resOff)
{
    ...
    if (sizeof(nehd) != LZRead(lzfd, (LPSTR)&nehd, sizeof(nehd)))
        return FALSE;
...
static BOOL find_pe_resource(HFILE lzfd, DWORD *resLen, DWORD *resOff, DWORD flags)
{
    ...
    if (len < sizeof(pehd.nt32.FileHeader))
        return FALSE;
    if (len < sizeof(pehd))
        memset((char *)&pehd + len, 0, sizeof(pehd) - len);
    ...
    for (i = 0; i < nSections; i++)
        if (resDataDir->VirtualAddress >= sections[i].VirtualAddress && resDataDir->VirtualAddress < sections[i].VirtualAddress +
    ...
    if (data_size != LZRead(lzfd, (char *)resSection, data_size))
        goto done;
    if (data_size < section_size)
        memset((char *)resSection + data_size, 0, section_size - data_size);
    ...
    for (i = 0; i < nSections; i++)
        if (resData->OffsetToData >= sections[i].VirtualAddress && resData->OffsetToData < sections[i].VirtualAddress +
                                                                                               sections[i].SizeOfRawData)
            break;
...
static DWORD find_version_resource(HFILE lzfd, DWORD *reslen, DWORD *offset, DWORD flags)
{
    ...
    case IMAGE_OS2_SIGNATURE:
        if (!find_ne_resource(lzfd, reslen, offset))
            magic = 0;
        break;
    case IMAGE_NT_SIGNATURE:
        if (!find_pe_resource(lzfd, reslen, offset, flags))
            magic = 0;
        break;
    }
    return magic;
...
static void print_vffi_debug(const VS_FIXEDFILEINFO *vffi)
{
    ...
    case VOS_UNKNOWN:
    default:
        TRACE("UNKNOWN(0x%x),", vffi->dwFileOS & 0xFFFF0000);
        break;
    }
    ...
    case VOS__PM32:
        ...
        break;
    default:
        TRACE("UNKNOWN(0x%x)", LOWORD(vffi->dwFileOS));
        break;
    }
    ...
    case VFT_DRV:
        ...
        case VFT2_DRV_VERSIONED_PRINTER:
            TRACE("VERSIONED_PRINTER");
            break;
        case VFT2_UNKNOWN:
        default:
            TRACE("UNKNOWN(0x%x)", vffi->dwFileSubtype);
            break;
        }
        break;
    case VFT_FONT:
    ...
    case VFT_VXD:
        ...
        break;
    case VFT_STATIC_LIB:
        TRACE("filetype=STATIC_LIB");
        break;
    case VFT_UNKNOWN:
    default:
        TRACE("filetype=Unknown(0x%x)", vffi->dwFileType);
        break;
    }
...
BOOL WINAPI GetFileVersionInfoExW(DWORD flags, LPCWSTR filename, DWORD handle, DWORD datasize, LPVOID data)
{
    ...
    case IMAGE_NT_SIGNATURE:
        ...
        if (datasize >= len)
            memcpy((char *)data + vvis->wLength, signature, sizeof(signature));
        if (TRACE_ON(ver))
...
static int testFileExistenceW(const WCHAR *path, const WCHAR *file, BOOL excl)
{
    ...
    if (pathlen > 1)
    {
        if (filename[pathlen - 2] != '\\')
            strcpy(&filename[pathlen - 1], "\\");
    }
    else /* specify the current directory */
        strcpy(filename, ".\\");
    ...
    PUINT lpuDestDirLen)
{
    ...
    if (flags & VFFF_ISSHAREDFILE)
    {
        ...
        if (lpszFilename)
        {
            if (testFileExistenceA(destDir, lpszFilename, FALSE))
                curDir = destDir;
                          ...
                          LPWSTR lpszDestDir, PUINT lpuDestDirLen)
{
    ...
    if (flags & VFFF_ISSHAREDFILE)
    {
        ...
        if (lpszFilename)
        {
            if (testFileExistenceW(destDir, lpszFilename, FALSE))
                curDir = destDir;
