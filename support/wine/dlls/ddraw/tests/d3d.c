...
static BOOL colortables_check_equality(PALETTEENTRY table1[256], PALETTEENTRY table2[256])
{
    ...
    for (i = 0; i < 256; i++)
    {
        if (table1[i].peRed != table2[i].peRed || table1[i].peGreen != table2[i].peGreen ||
            table1[i].peBlue != table2[i].peBlue)
            return FALSE;
    }
...
static void DeviceLoadTest(void)
{
            ...
            for (x = 0; x < ddsd.dwWidth; x++)
            {
                ...
                else
                {
                    ...
                    if (r != 0xff || !IS_VALUE_NEAR(g, x + loadrect.left - loadpoint.x) ||
                        !IS_VALUE_NEAR(b, y + loadrect.top - loadpoint.y))
                        diff_count2++;
                }
            }
        }
    ...
    */
    ...
    for (i = 0; i < 2; i++)
    {
        for (i1 = 7; i1 >= 0; i1--)
        {
            if (texture_levels[i][i1])
                IDirectDrawSurface7_Release(texture_levels[i][i1]);
        }
    }
    memset(texture_levels, 0, sizeof(texture_levels));
    ...
    else
    {
        ...
        for (i = 0; i < 2; i++)
            for (i1 = 5; i1 >= 0; i1--)
                for (i2 = 7; i2 >= 0; i2--)
                {
                    if (cube_face_levels[i][i1][i2])
                        IDirectDrawSurface7_Release(cube_face_levels[i][i1][i2]);
                }
        memset(cube_face_levels, 0, sizeof(cube_face_levels));
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        if (U2(ddsd).dwMipMapCount != (i ? 4 : 8))
            goto out;
...
out:
    ...
    {
        if (palettes[i])
            IDirectDrawPalette_Release(palettes[i]);
    }
...
static HRESULT WINAPI enum_z_fmt_cb(DDPIXELFORMAT *fmt, void *ctx)
{
    static const DDPIXELFORMAT formats[] =
        {
            {sizeof(DDPIXELFORMAT), DDPF_ZBUFFER, 0, {16}, {0}, {0x0000ffff}, {0x00000000}, {0x00000000}},
            {sizeof(DDPIXELFORMAT), DDPF_ZBUFFER, 0, {32}, {0}, {0xffffff00}, {0x00000000}, {0x00000000}},
            {sizeof(DDPIXELFORMAT), DDPF_ZBUFFER | DDPF_STENCILBUFFER, 0, {32}, {8}, {0xffffff00}, {0x000000ff}, {0x00000000}},
            {sizeof(DDPIXELFORMAT), DDPF_ZBUFFER, 0, {32}, {0}, {0x00ffffff}, {0x00000000}, {0x00000000}},
            {sizeof(DDPIXELFORMAT), DDPF_ZBUFFER | DDPF_STENCILBUFFER, 0, {32}, {8}, {0x00ffffff}, {0xff000000}, {0x00000000}},
            {sizeof(DDPIXELFORMAT), DDPF_ZBUFFER, 0, {24}, {0}, {0x00ffffff}, {0x00000000}, {0x00000000}},
            {sizeof(DDPIXELFORMAT), DDPF_ZBUFFER, 0, {32}, {0}, {0xffffffff}, {0x00000000}, {0x00000000}},
        };
     ...
     * Radeon 9000M WinXP) */
    if (U1(*fmt).dwZBufferBitDepth == 24)
        expected_pitch = ddsd.dwWidth * 4;
    else
        expected_pitch = ddsd.dwWidth * U1(*fmt).dwZBufferBitDepth / 8;
    ...
    if (U1(ddsd).lPitch != 0 && U1(ddsd).lPitch != expected_pitch && !broken(U1(ddsd).lPitch == expected_pitch + 128))
    {
    ...
    {
        if (memcmp(&formats[i], fmt, fmt->dwSize) == 0)
            return DDENUMRET_OK;
    }
