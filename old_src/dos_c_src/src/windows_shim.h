...
// SHORT GetKeyState(nVirtKey: i16);
...
// HANDLE CreateFileA(LPCSTR lpFileName, dwDesiredAccess: u32, dwShareMode: u32, LPSECURITY_ATTRIBUTES lpSecurityAttributes, dwCreationDisposition: u32, dwFlagsAndAttributes: u32, HANDLE hTemplateFile);
DWORD GetLastError();


// BOOL ReadFile(HANDLE hFile, LPVOID lpBuffer, nNumberOfBytesToRead: u32, Lu32* lpNumberOfBytesRead, LPOVERLAPPED lpOverlapped);
...
// BOOL CreateProcessA(LPCSTR lpApplicationName, LPSTR lpCommandLine, LPSECURITY_ATTRIBUTES lpProcessAttributes, LPSECURITY_ATTRIBUTES lpThreadAttributes, BOOL bInheritHandles, dwCreationFlags: u32, LPVOID lpEnvironment, LPCSTR
// lpCurrentDirectory, LPSTARTUPINFOA lpStartupInfo, LPPROCESS_INFORMATION lpProcessInformation); BOOL FindNextFileA(HANDLE hFindFile, LPWIN32_FIND_DATAA lpFindFileData); BOOL ScreenToClient(HWND hWnd, LPPOlpPoi16: i16); BOOL
// GetCursorPos(LPPOlpPoi16: i16); LRESULT DispatchMessageA(MSG *lpMsg); BOOL TranslateMessage(MSG *lpMsg); BOOL ReleaseCapture(void); HWND SetCapture(HWND hWnd); BOOL PeekMessageA(LPMSG lpMsg, HWND hWnd, wMsgFilterMin: u32, u32
...
// hCursor); void ExitProcess(u32 uExitCode); LRESULT DefWindowProcA(HWND hWnd, Msg: u32, WPARAM wParam, LPARAM lParam); void PostQuitMessage(nExitCode: i16); HWND CreateWindowExA(dwExStyle: u32, LPCSTR lpClassName, LPCSTR lpWindowName, u32
...
// GetSystemPaletteEntries(HDC hdc, iStart: u32, cEntries: u32, LPPALETTEENTRY pPalEntries); i16 GetDeviceCaps(HDC hdc, index: i16); HDC GetDC(HWND hWnd); BOOL FindClose(HANDLE hFindFile); HANDLE FindFirstFileA(LPCSTR lpFileName,
...
// HANDLE CreateThread(LPSECURITY_ATTRIBUTES lpThreadAttributes, SIZE_T dwStackSize, LPTHREAD_START_ROUTINE lpStartAddress, LPVOID lpParameter, dwCreationFlags: u32, Lu32* lpThreadId);
...
// i16 WideCharToMultiByte(CodePage: u32, dwFlags: u32, LPCWSTR lpWideCharStr, i16 cchWideChar, LPSTR lpMultiByteStr, i16 cbMultiByte, LPCSTR lpDefaultChar, LPBOOL lpUsedDefaultChar);
// BOOL WriteConsoleA(HANDLE hConsoleOutput, void *lpBuffer, nNumberOfCharsToWrite: u32, Lu32* lpNumberOfCharsWritten, LPVOID lpReserved);
...
// i16 MultiByteToWideChar(CodePage: u32, dwFlags: u32, LPCSTR lpMultiByteStr, i16 cbMultiByte, LPWSTR lpWideCharStr, cchWideChar: i16);
...
// LPVOID VirtualAlloc(LPVOID lpAddress, SIZE_T dwSize, flAllocationType: u32, u32 flProtect);
// BOOL WriteFile(HANDLE hFile, LPCVOID lpBuffer, nNumberOfBytesToWrite: u32, Lu32* lpNumberOfBytesWritten, LPOVERLAPPED lpOverlapped);
