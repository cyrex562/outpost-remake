...
static void test_WsReadType(void)
{
    ...
    if (val_str)
        ok(!lstrcmpW(val_str, testW), "wrong data\n");
    ...
    hr = WsReadType(reader, WS_ELEMENT_CONTENT_TYPE_MAPPING, WS_u16YPE, NULL,
...
static void test_WsGetXmlAttribute(void)
{
    ...
    if (str)
        ok(!memcmp(str, valueW, count * sizeof(WCHAR)), "wrong data\n");
...
static void test_simple_struct_type(void)
{
    ...
    if (test)
    {
        ...
        if (test->str)
            ok(!lstrcmpW(test->str, testW), "wrong data\n");
    }
    ...
    if (test)
    {
        ok(test->str != NULL, "str not set\n");
        if (test->str)
            ok(!lstrcmpW(test->str, testW), "wrong data test %p test->str %p\n", test, test->str);
    }
...
static void test_cdata(void)
{
    ...
    ok(hr == S_OK, "got %08x\n", hr);
    if (node)
        ok(node->nodeType == WS_XML_NODE_TYPE_CDATA, "got %u\n", node->nodeType);
    ...
    ok(hr == S_OK, "got %08x\n", hr);
    if (node)
        ok(node->nodeType == WS_XML_NODE_TYPE_END_CDATA, "got %u\n", node->nodeType);
...
{
    const WS_XML_NODE *node;
    if (WsReadNode(reader, NULL) != S_OK)
        return NULL;
    if (WsReadNode(reader, NULL) != S_OK)
        return NULL;
    if (WsGetReaderNode(reader, &node, NULL) != S_OK)
        return NULL;
