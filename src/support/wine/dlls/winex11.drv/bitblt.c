...
void execute_rop(X11DRV_PDEVICE *physdev, Pixmap src_pixmap, GC gc, const RECT *visrect, DWORD rop)
{
  ...
  if (use_dst)
    BITBLT_GetDstArea(physdev, pixmaps[DST], gc, visrect);
  null_brush = use_pat && !X11DRV_SetupGCForPatBlt(physdev, gc, TRUE);

  for (; *opcode; opcode++)
  {
    if (OP_DST(*opcode) == DST)
      result = pixmaps[DST];
  ...
  if (pixmaps[TMP])
    XFreePixmap(gdi_display, pixmaps[TMP]);
...
BOOL X11DRV_PatBlt(PHYSDEV dev, struct bitblt_coords *dst, DWORD rop)
{
  ...
  if (usePat && !X11DRV_SetupGCForBrush(physDev))
    return TRUE;
...
{
  return get_dib_stride(info->bmiHeader.biWidth, info->bmiHeader.biBitCount) * abs(info->bmiHeader.biHeight);
}
...
static BOOL matching_color_info(const XVisualInfo *vis, const BITMAPINFO *info)
{
  ...
  case 8:
  {
    ...
    if (count != info->bmiHeader.biClrUsed)
      return FALSE;
    for (i = 0; i < count; i++)
    {
...
static inline BOOL image_needs_byteswap(XImage *image, BOOL is_r8g8b8, int bit_count)
{
  ...
  {
  case 1:
    return image->bitmap_bit_order != MSBFirst;
  case 4:
    return image->byte_order != MSBFirst;
  case 16:
  case 32:
    return image->byte_order != client_byte_order;
  case 24:
    return (image->byte_order == MSBFirst) ^ !is_r8g8b8;
  default:
    return FALSE;
  }
}
                      ...
                      struct bitblt_coords *coords, const int *mapping, unsigned int zeropad_mask)
{
  ...
  else
  {
    ...
    dst_bits->free = NULL;
    if (!need_byteswap && zeropad_mask == ~0u && !mapping)
                      ...
                      struct bitblt_coords *dst, DWORD rop)
{
  ...
  if (info->bmiHeader.biPlanes != 1)
    goto update_format;
  if (info->bmiHeader.biBitCount != format->bits_per_pixel)
    goto update_format;
  /* FIXME: could try to handle 1-bpp using XCopyPlane */
  if (!matching_color_info(&vis, info))
    goto update_format;
  if (!bits)
    return ERROR_SUCCESS; /* just querying the format */
                      ...
                      struct gdi_image_bits *bits, struct bitblt_coords *src)
{
  ...
  case 4:
    align = 8;
    mapping = X11DRV_PALETTE_XPixelToPalette;
    break;
  case 8:
    align = 4;
    mapping = X11DRV_PALETTE_XPixelToPalette;
  ...
  if (!bits)
    return ERROR_SUCCESS; /* just querying the color information */
...
static void update_surface_region(struct x11drv_window_surface *surface)
{
  ...
  UINT *masks = info->bmiColors;
  int x, y, start, width;
  HRGN rgn;

  if (!shape_layered_windows)
    return;
  ...
  case 16:
  {
    ...
    for (y = surface->header.rect.top; y < surface->header.rect.bottom; y++, bits += stride)
    {
      ...
      while (x < width)
      {
        while (x < width && (bits[x] & mask) == surface->color_key)
          x++;
        start = x;
        while (x < width && (bits[x] & mask) != surface->color_key)
          x++;
        ...
        while (x < width)
        {
          while (x < width &&
                 ((bits[x] & 0xffffff) == surface->color_key ||
                  (surface->is_argb && !(bits[x] & 0xff000000))))
            x++;
          start = x;
          while (x < width &&
                 !((bits[x] & 0xffffff) == surface->color_key ||
...
static XImage *create_shm_image(const XVisualInfo *vis, int width, int height, XShmSegmentInfo *shminfo)
{
  ...
  if (image->bytes_per_line & 3)
    goto failed; /* we need 32-bit alignment */
...
static void x11drv_surface_set_region(struct window_surface *window_surface, HRGN region)
{
  ...
  {
    if (!surface->region)
      surface->region = CreateRectRgn(0, 0, 0, 0);
...
static void x11drv_surface_destroy(struct window_surface *window_surface)
{
  ...
  if (surface->gc)
    XFreeGC(gdi_display, surface->gc);
  if (surface->image)
  {
    if (surface->image->data != surface->bits)
                                      ...
                                      COLORREF color_key, BOOL use_alpha)
{
  ...
  if (surface->byteswap || format->bits_per_pixel == 4 || format->bits_per_pixel == 8)
  {
    ...
    surface->bits = surface->image->data;
...
HRGN expose_surface(struct window_surface *window_surface, const RECT *rect)
{
  ...
  if (window_surface->funcs != &x11drv_surface_funcs)
    return 0; /* we may get the null surface */
