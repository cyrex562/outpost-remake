...
{
    INT index;
    if (offset < 0 && offset % 2 == 0 && ((index = -(offset + 8) / 2) < ARRAY_SIZE(ClassLongOffsetNames)))
    {
...
static void SPY_GetMsgStuff(SPY_INSTANCE *sp_e)
{
    ...
    if (!msg_name)
    {
        ...
        while (cc_array[i].classname &&
               strcmpiW(cc_array[i].classname, sp_e->wnd_class) != 0)
            i++;
...
static void SPY_DumpStructure(const SPY_INSTANCE *sp_e, BOOL enter)
{
    ...
    case WM_NOTIFY:
        /* if (!enter) break; */
        {
            ...
            if (p)
            {
                ...
                else if (pnmh->code >= HDN_ENDDRAG && pnmh->code <= HDN_ITEMCHANGINGA)
                {
    ...
    default:
        if (sp_e->data_len > 0)
            SPY_DumpMem("MSG lParam", sp_e->lParam, sp_e->data_len);
    }
}
                      ...
                      WPARAM wParam, LPARAM lParam)
{
    ...
    case SPY_SENDMESSAGE:
    {
        ...
        if (tid == GetCurrentThreadId())
            strcpy(taskName, "self");
        else
            sprintf(taskName, "tid %04x", GetCurrentThreadId());
